...
function renderHero(game){
  ...
  let preview=heroNode?.querySelector('.game-hero__preview');if(heroNode&&!preview){preview=document.createElement('img');preview.className='game-hero__preview';preview.alt='';preview.decoding='async';heroNode.prepend(preview)}
  const setHero=(url,button)=>{if(url&&preview)preview.src=url;document.querySelectorAll('.hero-media__item').forEach(item=>item.classList.toggle('active',item===button))};
  setHero(hero,null);
...
function renderOverview(game,catalog){
  ...
  const editorial=Number(game.ratings.igropoisk),users=Number(game.ratings.users),votes=Number(game.ratings.user_votes||0),editorialValue=Number.isFinite(editorial)?Math.max(0,Math.min(10,editorial)):0,userValue=votes>0&&Number.isFinite(users)?Math.max(0,Math.min(10,users)):0;
  document.querySelector('#ratingList').innerHTML=`<div><span>Игропоиск</span><progress class="rating-meter" max="10" value="${editorialValue}"></progress><strong>${formatScore(editorial)}</strong></div><div><span>Игроки</span><progress class="rating-meter" max="10" value="${userValue}"></progress><strong class="ig-score-user">${votes>0?formatScore(users):'—'}</strong></div>`;
...
function renderAwards(awards){const valid=arr(awards).filter(item=>item?.name&&(item.source_url||item.url));const panel=document.querySelector('#awardsPanel');panel.hidden=!valid.length;document.querySelector('#awards').innerHTML=valid.map(item=>`<a class="award-source award-source--single" href="${esc(item.source_url||item.url)}" target="_blank" rel="noopener noreferrer"><div><b>${esc(item.name)}</b>${item.category?`<span>${esc(item.category)}</span>`:''}<small>${esc([item.year,item.source_name||item.source].filter(Boolean).join(' · '))} ↗</small></div></a>`).join('')}
...
function bindRating(title,runtime,game){
  ...
  dialogTitle.textContent=title;scale.innerHTML=Array.from({length:10},(_,index)=>`<button class="ig-button rating-scale__button" type="button" data-rating="${index+1}">${index+1}</button>`).join('');
