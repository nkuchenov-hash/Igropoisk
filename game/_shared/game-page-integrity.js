...
const score=s=>{const a=Number(s?.score),b=Number(s?.scale);if(Number.isFinite(a)&&Number.isFinite(b)&&b>0)return `${a}/${b}`;if(s?.original_score?.display)return String(s.original_score.display);if(Number.isFinite(Number(s?.normalized_10)))return `${Number(s.normalized_10).toFixed(1).replace(/\.0$/,'')}/10`;return '—'};
...
const directReview=s=>{const roles=arr(s?.roles).map(String);const kind=String(s?.kind||'');const u=sourceUrl(s),t=String(s?.title||'');if(!u||blockedReviewDomain(u)||hubLike(u))return false;if(!roles.includes('review')&&kind!=='professional-review'&&String(s?.source_kind||'')!=='review')return false;if(/пользовательские отзывы|user reviews?/i.test(t))return false;return true};
...
const normalizeLegacyReviews=feed=>arr(feed?.reviews).map(s=>({...s,kind:'professional-review',roles:['review','rating'],name:s.publication||s.name||s.domain||'Источник'}));
...
function hideOwnGuidePresentation(){const featured=document.querySelector('#featuredGuide');if(featured){featured.hidden=true;featured.setAttribute('aria-hidden','true')}const quick=document.querySelector('#guideQuickLinks')?.closest('section');if(quick){quick.hidden=true;quick.setAttribute('aria-hidden','true')}const updated=document.querySelector('#guideUpdated')?.closest('section');if(updated){updated.hidden=true;updated.setAttribute('aria-hidden','true')}const layout=document.querySelector('#guides .guides-layout');if(layout)layout.style.gridTemplateColumns='1fr'}
let ratingControlsRemovalScheduled=false;
function scheduleDisabledRatingControlsRemoval(){if(ratingControlsRemovalScheduled)return;ratingControlsRemovalScheduled=true;let attempts=0,hydratedAt=0;const timer=setInterval(()=>{attempts++;const dialog=document.querySelector('#ratingDialog');if(!dialog){clearInterval(timer);return}const hydrated=document.querySelectorAll('#ratingScale button').length===10&&Boolean(document.querySelector('#ratingDialogTitle')?.textContent?.trim());if(hydrated&&!hydratedAt){hydratedAt=Date.now();return}if(!hydrated&&attempts<160)return;if(hydrated&&Date.now()-hydratedAt<1200&&attempts<160)return;clearInterval(timer);for(const selector of ['#rateGame','#rateInline','#ratingDialog'])document.querySelector(selector)?.remove()},50)}
function renderFranchise(franchise){const games=arr(franchise?.games).filter(game=>game?.title&&game?.slug);if(franchise?.status!=='green'||!franchise?.name||!games.length)return;const overview=document.querySelector('#overview .lower-grid');if(!overview)return;let panel=document.querySelector('#franchisePanel');if(!panel){panel=document.createElement('section');panel.id='franchisePanel';panel.className='ig-panel game-panel franchise-panel';overview.appendChild(panel)}panel.innerHTML=`<div class="ig-toolbar franchise-panel__head"><div><h2>Игры серии</h2><span>${esc(franchise.name)}</span></div></div><div class="franchise-row">${games.map(game=>`<a class="ig-card franchise-game" href="../${encodeURIComponent(game.slug)}/"><b>${esc(game.title)}</b><span>${esc(game.release_year||game.year||'')}</span></a>`).join('')}</div>`}
async function main(){const[draft,corpus,ratings,editorial,legacyReviews,franchise]=await Promise.all([fetchJSON(`../../data/drafts/${encodeURIComponent(slug)}.json`),fetchJSON(`../../data/game-sources/${encodeURIComponent(slug)}.json`),fetchJSON(`../../data/ratings/${encodeURIComponent(slug)}.json`),fetchJSON(`../../data/page-editorial/${encodeURIComponent(slug)}.json`),fetchJSON(`../../data/reviews/${encodeURIComponent(slug)}.json`),fetchJSON(`../../data/franchises/${encodeURIComponent(slug)}.json`)]);for(let i=0;i<120&&!document.querySelector('#reviewGrid');i++)await new Promise(r=>setTimeout(r,100));const apply=()=>{ownReviewGrid(corpus,ratings,legacyReviews);renderAllReviews(corpus,ratings,legacyReviews);applyPresentation(draft,editorial);renderReviewSummaryCard(draft,ratings,editorial);fixOfficialLinks(draft);hideOwnGuidePresentation();scheduleDisabledRatingControlsRemoval();renderFranchise(franchise)};apply();setTimeout(apply,400);setTimeout(apply,1200);setTimeout(apply,2500);setTimeout(apply,5000)}
