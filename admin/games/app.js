...
function render(){
  const games=filtered(); el('summary').innerHTML=[['Всего игр',state.games.length],['В очереди',state.games.filter(game=>!game.published&&!['rejected','merged_into_another_game'].includes(game.status)).length],['Конфликты',state.games.filter(game=>game.conflicts).length],['Неоднозначные',state.reviewQueue.length],['Опубликовано',state.games.filter(game=>game.published).length]].map(([label,value])=>`<article class="ig-admin-card"><div class="ig-kicker">${esc(label)}</div><h2>${value}</h2></article>`).join('');
  el('rows').innerHTML=games.map(game=>`<tr><td><strong>${esc(game.title)}</strong><br><code>${esc(game.id??'ID pending')}</code><br><small>${esc(game.slug)} · ${esc(game.kind??'game')}</small></td><td><strong>${esc(game.status)}</strong><div>${esc(game.pageStatus??'')} / ${esc(game.reviewStatus??'')}</div><div class="status-reason">${esc(game.statusReason??'')}</div></td><td><div>${Math.round((game.completeness??0)*100)}%</div><div class="meter"><span style="width:${Math.round((game.completeness??0)*100)}%"></span></div></td><td><strong>${game.priority?.score??0}</strong><div class="status-reason">${esc((game.priority?.reasons??[]).slice(0,3).map(item=>item.signal).join(', '))}</div></td><td>${game.sourceCount??0} источн.<br>${game.conflicts??0} конфликт.<br>${game.possibleDuplicates??0} возможн. дублей<br>${game.lockedFields?.length??0} блокировок</td><td><div class="games-actions"><a class="ig-button secondary" href="${esc(game.publicUrl??'#')}" target="_blank">Предпросмотр</a><button class="ig-button secondary" data-action="history" data-id="${esc(game.id)}">История</button><button class="ig-button secondary" data-action="reenrich" data-id="${esc(game.id)}" ${state.apiWritable?'':'disabled'}>Обогатить</button><button class="ig-button secondary" data-action="lock" data-id="${esc(game.id)}" ${state.apiWritable?'':'disabled'}>Блокировать поле</button><button class="ig-button secondary" data-action="merge" data-id="${esc(game.id)}" ${state.apiWritable?'':'disabled'}>Объединить</button>${game.status==='merged_into_another_game'?`<button class="ig-button secondary" data-action="undo-merge" data-id="${esc(game.id)}" ${state.apiWritable?'':'disabled'}>Отменить merge</button>`:''}<button class="ig-button" data-action="publish" data-id="${esc(game.id)}" ${state.apiWritable?'':'disabled'}>${game.published?'Снять':'Опубликовать'}</button></div></td></tr>`).join('');
}
