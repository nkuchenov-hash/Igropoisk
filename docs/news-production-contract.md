...
Отбор выполняется на уровне отдельного события. Событие может не попасть в публичную выборку, если оно не прошло порог значимости/популярности или финальную проверку собственного публичного текста.
...
- дефект локализации/редактуры отдельного события;
- диагностическая quality/health проверка.

Такие проблемы отражаются в reports/health как warning/degraded и исправляются отдельно. Если публичный текст конкретного события не удаётся автоматически исправить до приемлемого состояния, исключается только это событие.
...
Ни source processing, ни editorial processing, ни финальный snapshot не имеют фиксированной квоты количества событий. Если интерфейсу когда-либо нужен отдельный лимит карточек, он задаётся как явная UI-настройка и не становится production-критерием pipeline.

## 4. Русская версия и финальный quality gate

- Русский публичный текст формируется pipeline.
- Локальная OPUS-MT (`Xenova/opus-mt-en-ru`) и удалённые переводчики могут создавать **черновую** EN→RU локализацию, но машинный перевод сам по себе не является готовым редакционным текстом.
- До публикации фактические `titleRu` и `summaryRu` каждого публичного события проходят финальную проверку независимо от общего количества новостей в цикле.
- Имена игр, студий, компаний, сервисов и мероприятий извлекаются из источника и защищаются от буквального перевода; явно заданные curated Russian display names остаются допустимы.
- Проверяется сохранение критических смысловых отрицаний/противопоставлений, собственных имён, отсутствие HTML-артефактов, оборванных латинских хвостов, повторов и других машинных дефектов.
- Подозрительный черновик автоматически переписывается редакторским AI по исходному английскому материалу и, когда возможно, по извлечённому тексту статьи.
- Исправленная версия снова проходит production + semantic validation.
- Если исправить конкретный материал не удалось, он исключается из публичного набора без остановки остальных событий.
...
2. сформировать русскую редакционную версию;
3. выполнить финальную per-item проверку публичного текста;
4. enqueue missing games независимо;
5. publish snapshot + switch live manifest;
6. prune old snapshots/media после публикации.
...
- `scripts/test-news-publication-quality.mjs`;
...
- превращать машинный перевод в автоматическое разрешение на публикацию без финального per-item quality gate;
