...
S = requests.Session()
S.headers.update({"User-Agent":"Mozilla/5.0 (compatible; IgropoiskBot/2.1; +https://github.com/nkuchenov-hash/Igropoisk)","Accept-Language":"en-US,en;q=.8,ru;q=.7"})
TIMEOUT = 25
...
def search_links(query, limit=10):
    out=[]
    # DuckDuckGo HTML
    try:
    ...
    # Bing RSS fallback: substantially more reliable in GitHub Actions than DDG alone
    try:
        r=S.get("https://www.bing.com/search",params={"q":query,"format":"rss"},timeout=TIMEOUT); r.raise_for_status()
        soup=BeautifulSoup(r.text,"xml")
        for item in soup.find_all("item"):
            u=clean(item.link.get_text() if item.link else "")
            if u.startswith("http") and u not in out: out.append(u)
...
def resolve_steam(title):
    ...
    for x in data.get("items",[]):
        name=x.get("name",""); sc=title_score(title,name)
        # Prevent false matches such as Hades -> Hades II or Overwatch -> unrelated titles.
        exactish = norm(title)==norm(name) or (sc>=94 and abs(len(norm(title))-len(norm(name)))<=10)
        if exactish and (not best or sc>best[0]): best=(sc,int(x["id"]))
...
def wiki_data(title):
    try:
        sr=get_json("https://en.wikipedia.org/w/api.php",{"action":"query","list":"search","srsearch":f'"{title}" video game',"format":"json","utf8":1})
        hits=sr.get("query",{}).get("search",[])
        if not hits:return {},None
        best=max(hits[:5],key=lambda h:title_score(title,h.get("title","")))
        if title_score(title,best.get("title",""))<78:return {},None
        p=get_json("https://en.wikipedia.org/api/rest_v1/page/summary/"+quote_plus(best["title"].replace(" ","_")))
        url=((p.get("content_urls") or {}).get("desktop") or {}).get("page","")
        src={"url":url,"domain":"en.wikipedia.org","source_name":"Wikipedia","title":p.get("title",best["title"]),"description":clean(p.get("extract",""))[:1400],"published_at":"","checked_at":now(),"type":"database","trust":.82,"supports":["identity","description","context"],"fingerprint":fp(url),"match_score":title_score(title,p.get("title",""))}
        return {"title":p.get("title",title),"short":clean(p.get("description","")),"about":clean(p.get("extract","")),"cover":((p.get("thumbnail") or {}).get("source",""))},src
...
def page_meta(url,adapter,title):
    try:
        ...
        score=max(title_score(title,ttl),title_score(title,desc[:240]))
...
def discover(title,official=""):
    found=[]; seen=set()
    if official:
        m=page_meta(official,REGISTRY[0],title)
        if m:found.append(m);seen.add(m["fingerprint"])
    for a in REGISTRY[1:]:
        candidates=[]
        for q in a.search_queries(title):
            for u in search_links(q,10):
                m=page_meta(u,a,title)
                if m and m["fingerprint"] not in seen:candidates.append(m)
            time.sleep(.15)
        candidates.sort(key=lambda x:x["match_score"],reverse=True)
        for m in candidates[:2 if a.kind=="editorial" else 1]:
...
def validate(g):
    e=[]
    checks=[(g["identity"].get("title"),"title"),(g["release"].get("date_text"),"release date"),(g["companies"].get("developers"),"developer"),(g["classification"].get("genres"),"genres"),(g["editorial"].get("short_description"),"short description"),(g["editorial"].get("integrated_description"),"integrated description"),(g["media"].get("cover"),"cover"),(g["media"].get("hero"),"hero")]
    for v,n in checks:
...
def build(seed):
    ...
    wiki,wsrc=wiki_data(seed)
    title=st.get("title") or wiki.get("title") or seed
    sources=discover(seed,st.get("official", ""))
    if wsrc and wsrc["fingerprint"] not in {x["fingerprint"] for x in sources}:sources.append(wsrc)
    store=st.get("store")
    ...
    short=st.get("short") or wiki.get("short") or wiki.get("about","")[:320]
    integrated=st.get("about") or wiki.get("about","")
    cover=st.get("cover") or wiki.get("cover","")
    g={"publication":{"status":"collecting","gate_passed":False,"errors":[]},"identity":{"slug":slugify(seed),"title":title,"seed_title":seed,"steam_appid":appid},"release":{"date_text":st.get("release", "")},"companies":{"developers":st.get("developers",[]),"publishers":st.get("publishers",[])},"classification":{"genres":st.get("genres",[]),"categories":st.get("categories",[]),"platforms":st.get("platforms",[])},"editorial":{"short_description":short,"integrated_description":integrated,"features":(st.get("categories") or st.get("genres") or [])[:8]},"media":{"cover":cover,"hero":st.get("hero", ""),"screenshots":st.get("screenshots",[]),"videos":st.get("videos",[])},"ratings":{"igropoisk":round(st["metacritic"]/10,1) if st.get("metacritic") else None,"users":None,"user_votes":0},"links":{"official":st.get("official", ""),"store":store or ""},"materials":{"reviews":reviews[:8],"news":[],"guides":[]},"sources":sorted(sources,key=lambda x:(x["trust"],x["match_score"]),reverse=True)}
    validate(g);return g
...
def main():
    ...
    for seed in seeds:
        ...
        (dd/f"{slug}.json").write_text(json.dumps(g,ensure_ascii=False,indent=2),encoding="utf-8")
    ...
    (pd/"games.json").write_text(json.dumps(public,ensure_ascii=False,indent=2),encoding="utf-8");report["finished_at"]=now();(ROOT/"data/parser-report.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
