const titleRejectPatterns = [
  ...
  /\d+(?:[.,]\d+)?%\s+игрок\p{L}*.{0,120}(?:достижен\p{L}*|первым\s+делом|оторвал\p{L}*)/iu,
  /\bachievement\b.{0,100}\b(?:players?|percent|%)\b|\b(?:players?|percent|%)\b.{0,100}\bachievement\b/i,
  /(?:^|[^\p{L}\p{N}])официально\s+вс[её](?:$|[^\p{L}\p{N}])/iu,
  /(?:^|[^\p{L}\p{N}])как\s+обновлен\p{L}*.{0,90}превратит.{0,90}настоящ\p{L}*\s+симулятор\p{L}*/iu
];
...
const textRejectPatterns = [
  ...
  /(?:^|[^\p{L}\p{N}])это\s+(?:отличн\p{L}*|хорош\p{L}*|плох\p{L}*)\s+новост\p{L}*/iu,
  /(?:^|[^\p{L}\p{N}])игра\s+бросает\s+вызов\s+любител\p{L}*/iu,
  /(?:^|[^\p{L}\p{N}])первый\s+(?:полноценн\p{L}*\s+)?обновлени\p{L}*/iu,
  /(?:^|[^\p{L}\p{N}])из\s+(?:первой|второй|третьей|четв[её]ртой|пятой|шестой|седьмой|восьмой|девятой|десятой)\s+эпизод\p{L}*/iu,
  /(?:^|[^\p{L}\p{N}])без\s+диск[- ]поддержк\p{L}*/iu,
  /(?:^|[^\p{L}\p{N}])показател\p{L}*\s+уронов(?=$|[^\p{L}\p{N}])/iu,
  /(?:^|[^\p{L}\p{N}])развернул\p{L}*\s+фокус(?=$|[^\p{L}\p{N}])/iu,
  /(?:геймпле\p{L}*|gameplay).{0,55}(?:на|on)\s+(?:Netflix|Hulu|Prime\s+Video)\b/iu
];
