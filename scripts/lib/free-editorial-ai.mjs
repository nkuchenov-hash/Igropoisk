...
export async function generateFreeEditorialJSON({system='',prompt,temperature=0.25}){
  ...
  try{
    ...
    const raw=stripFence(data?.message?.content||data?.response||'');
    if(!raw)throw new Error('Qwen/Ollama returned empty output');
    let parsed;
    try{parsed=JSON.parse(raw)}catch(error){throw new Error(`Qwen/Ollama returned invalid JSON: ${error.message}`)}
    return {data:parsed,provider:'ollama',model,baseUrl};
  }catch(error){
    if(error?.name==='AbortError')throw new Error(`Qwen/Ollama timed out after ${timeoutMs} ms`);
    throw error;
...
export async function assertFreeEditorialAI(){
  ...
  try{
    ...
    if(!response.ok)throw new Error(`Ollama health ${response.status}`);
    const payload=await response.json();
    const names=(payload?.models||[]).map(item=>String(item?.name||item?.model||''));
    const available=names.some(name=>name===model||name.startsWith(`${model}:`)||model.startsWith(`${name}:`));
    if(!available)throw new Error(`Required free model ${model} is not installed`);
