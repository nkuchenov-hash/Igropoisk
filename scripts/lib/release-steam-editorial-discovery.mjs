import { parseReleaseDateClaim } from './release-date-precision.mjs';
...
async function mapPool(items, concurrency, worker) {
  ...
  const runners = Array.from({length: Math.max(1, Math.min(concurrency, items.length || 1))}, async () => {
    while (cursor < items.length) { const index = cursor++; try { results[index] = await worker(items[index], index); } catch (error) { results[index] = {error}; } }
  });
  await Promise.all(runners); return results;
}
function parseSearchAppIds(html = '') {
  return uniq([...String(html).matchAll(/data-ds-appid="([^"]+)"/gi)].flatMap(match => String(match[1]).split(',')).map(value => Number(value.trim())).filter(Number.isFinite));
}
function mergeSignal(release, signal) {
  const quality = release?.editorial_quality || {};
  return {...release, editorial_quality: {...quality, signals: uniq([...(quality.signals || []), signal])}};
}

export function rawReleaseFromSteamDetails(data, appid, signals = [], checkedAt = new Date().toISOString()) {
  ...
  return {
    ...
    developer: (data.developers || [])[0] || '', publisher: (data.publishers || [])[0] || '',
    external_ids: {steam: appid, igdb: null, rawg: null},
    image: {source_url:`https://cdn.cloudflare.steamstatic.com/steam/apps/${appid}/library_600x900.jpg`,local_url:null,kind:'official_store_cover',width:600,height:900,verified:true,source_id:sourceId,status:'remote_verified'},
    events: [{
      id: `${slug}:worldwide:${dateIdentity}`,
      date: claim.date, date_start: claim.date_start, date_end: claim.date_end, precision: claim.precision,
      raw_date: claim.raw_date, region:'worldwide', platforms:['PC'], status:'confirmed', confidence:0.97, source_ids:[sourceId],
    }],
    sources: [{id:sourceId,family:'official_store',priority:90,title:'Steam',url:`https://store.steampowered.com/app/${appid}/`,checked_at:checkedAt,date_claim:claim.raw_date,status:'success'}],
    editorial: {status:'draft',readiness:45,needs_review:false,has_page:false,draft_path:`data/release-drafts/${slug}.json`,locked_fields:[],notes:[]},
    editorial_quality: {signals: uniq(signals)}, first_seen_at:checkedAt, last_seen_at:checkedAt,
  };
}
...
async function fetchSearchSignal({filter, sortBy, limit, strongSignal, discoverySignal, strongRank}) {
  ...
  try {
    const payload=await fetchJSON(url); const ids=parseSearchAppIds(payload?.results_html||'');
    return {ids,ranked:ids.map((appid,index)=>({appid,signal:index<strongRank?strongSignal:discoverySignal,rank:index+1})),status:'success',url,duration_ms:Date.now()-started,strong_signal:strongSignal,strong_rank:strongRank};
  } catch (error) {
    return {ids:[],ranked:[],status:'error',error:error.message,url,duration_ms:Date.now()-started,strong_signal:strongSignal,strong_rank:strongRank};
  }
}
...
export async function enrichRawReleasesFromSteamEditorial(rawReleases = [], policy = {}, generatedAt = new Date().toISOString()) {
  ...
  const now=Date.parse(generatedAt), lowerBound=now-recentDays*DAY_MS, upperBound=now+horizonDays*DAY_MS;
  const signals=await Promise.all([
    fetchSearchSignal({filter:'popularcomingsoon',sortBy:'Released_ASC',limit:upcomingLimit,strongSignal:'steam_popular_upcoming',discoverySignal:'steam_popular_upcoming_discovery',strongRank}),
    fetchSearchSignal({filter:'popularnew',sortBy:'Released_DESC',limit:recentLimit,strongSignal:'steam_popular_new',discoverySignal:'steam_popular_new_discovery',strongRank}),
  ]);
  ...
  const discovered=await mapPool(missing,8,async item=>{
    const payload=await fetchJSON(`https://store.steampowered.com/api/appdetails?appids=${item.appid}&cc=us&l=english`);const result=payload?.[item.appid];
    if(!result?.success||!result?.data)return null;
    const release=rawReleaseFromSteamDetails(result.data,item.appid,item.signals,generatedAt);if(!release)return null;
    const releaseTime=Date.parse(`${release.events[0].date_start}T12:00:00Z`);
