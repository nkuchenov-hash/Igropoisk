...
const STORE_FAMILIES = new Set(['official_store', 'store', 'steam', 'rawg']);
...
export function evaluateHomeReleaseQuality(game, options = {}) {
  ...
  const wishlistSignal = steamWishlistPosition > 0 && steamWishlistPosition <= Number(options.maximumSteamWishlistPosition || 100);
  const coverageCount = Math.max(Number(anticipation.independent_publication_count || 0), independentSources.length);
  const coverageFamilies = new Set([...(anticipation.evidence_families || []), ...independentSources.map(source => source.family)].filter(Boolean));
  const crossSiteSignal = coverageCount >= Number(options.minimumIndependentCoverage || 2) || coverageFamilies.size >= 2;
  ...
  let score = 0;
  if (popularSignal) score += 10;
  if (wishlistSignal) score += Math.max(2, 8 - Math.floor((steamWishlistPosition - 1) / 20));
  if (crossSiteSignal) score += Math.min(8, 3 + coverageCount);
  ...
  if (!signals.length) reasons.push('no_anticipation_signal');
  if (upcoming && !popularSignal && !wishlistSignal && !crossSiteSignal && !manualFeature) reasons.push('upcoming_without_measured_interest');
