...
if(!quickVerifier.includes("editorial_quality?.passed===true")||!quickVerifier.includes('process.exit(2)'))fail('Quick-review publication is not verified fail-closed');
...
for(const marker of ['POST_CREATE_PHASE: quick-review','POST_CREATE_PUBLISH_PHASE: quick-review','POST_CREATE_PHASE: full-review','POST_CREATE_PUBLISH_PHASE: full-review','scripts/verify-post-create-quick-reviews.mjs','Quick-review publication gate did not pass after bounded retries'])if(!enrichmentWorkflow.includes(marker))fail(`Independent fail-closed quick/full workflow checkpoint missing: ${marker}`);
const bootstrapAt=enrichmentWorkflow.indexOf('POST_CREATE_PHASE: bootstrap'),bootstrapPublishAt=enrichmentWorkflow.indexOf('POST_CREATE_PUBLISH_PHASE: bootstrap'),modelAt=enrichmentWorkflow.indexOf('Restore local editorial model cache'),quickAt=enrichmentWorkflow.indexOf('POST_CREATE_PHASE: quick-review'),quickPublishAt=enrichmentWorkflow.indexOf('POST_CREATE_PUBLISH_PHASE: quick-review'),visionAt=enrichmentWorkflow.indexOf('Ensure optional local vision model for full upgrades'),fullAt=enrichmentWorkflow.indexOf('POST_CREATE_PHASE: full-review'),fullPublishAt=enrichmentWorkflow.indexOf('POST_CREATE_PUBLISH_PHASE: full-review');
if(bootstrapAt<0||bootstrapPublishAt<bootstrapAt||modelAt<bootstrapPublishAt||quickAt<modelAt||quickPublishAt<quickAt||visionAt<quickPublishAt||fullAt<visionAt||fullPublishAt<fullAt)fail('Verified quick review is not published before optional vision and heavy full-review phase');
...
console.log('Game Creator post-create contract passed: event-scoped green ratings lead to a verified local quick review before independent local full upgrades.');
