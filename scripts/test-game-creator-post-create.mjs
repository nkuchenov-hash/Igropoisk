...
for(const marker of ['githubChatJson','GITHUB_EDITORIAL_MODEL',"requestedProvider==='local'",'commercial_depth_gate:true'])if(!commercialSynthesis.includes(marker))fail(`Commercial primary/fallback synthesis missing: ${marker}`);
for(const marker of ['factual_grounding','natural_russian','unsupportedNumbers','lowercase_latin_intrusions','githubChatJson','GITHUB_AUDIT_MODEL',"requestedProvider==='local'",'process.exit(2)'])if(!quickAudit.includes(marker))fail(`Quick-review factual/language audit missing: ${marker}`);
for(const marker of ['https://models.github.ai/inference/chat/completions','GITHUB_TOKEN','openai/gpt-4.1'])if(!githubModel.includes(marker))fail(`GitHub commercial model client missing: ${marker}`);
...
for(const marker of ["OPENAI_API_KEY: ''",'models: read','GITHUB_REVIEW_MODEL: openai/gpt-4.1','GITHUB_AUDIT_MODEL: openai/gpt-4.1','EDITORIAL_PROVIDER: github','qwen3:4b','qwen3-vl:4b'])if(!enrichmentWorkflow.includes(marker))fail(`Commercial primary/fallback model routing missing: ${marker}`);
for(const marker of ['Ensure required text model for quick review','for attempt in 1 2 3 4 5','Required text model','Ensure optional local vision model for full upgrades','Optional vision model','published quick review remains valid'])if(!enrichmentWorkflow.includes(marker))fail(`Local fallback/vision resilience missing: ${marker}`);
if(enrichmentWorkflow.includes('GITHUB_VISION_MODEL'))fail('GitHub text primary is incorrectly coupled to vision');
...
for(const marker of ['build-review-bootstrap-commercial.mjs','POST_CREATE_PUBLISH_PHASE: quick-review','POST_CREATE_PHASE: full-review','POST_CREATE_PUBLISH_PHASE: full-review','scripts/verify-post-create-quick-reviews.mjs','Grounded quick-review publication gate did not pass after two bounded attempts'])if(!enrichmentWorkflow.includes(marker))fail(`Independent grounded quick/full workflow checkpoint missing: ${marker}`);
const bootstrapAt=enrichmentWorkflow.indexOf('POST_CREATE_PHASE: bootstrap'),bootstrapPublishAt=enrichmentWorkflow.indexOf('POST_CREATE_PUBLISH_PHASE: bootstrap'),modelAt=enrichmentWorkflow.indexOf('Restore local editorial model cache'),quickAt=enrichmentWorkflow.indexOf('Build, fact-check and verify bounded quick reviews'),quickPublishAt=enrichmentWorkflow.indexOf('POST_CREATE_PUBLISH_PHASE: quick-review'),visionAt=enrichmentWorkflow.indexOf('Ensure optional local vision model for full upgrades'),fullAt=enrichmentWorkflow.indexOf('POST_CREATE_PHASE: full-review'),fullPublishAt=enrichmentWorkflow.indexOf('POST_CREATE_PUBLISH_PHASE: full-review');
if(bootstrapAt<0||bootstrapPublishAt<bootstrapAt||modelAt<bootstrapPublishAt||quickAt<modelAt||quickPublishAt<quickAt||visionAt<quickPublishAt||fullAt<visionAt||fullPublishAt<fullAt)fail('Verified grounded quick review is not published before optional vision and heavy full-review phase');
...
console.log('Game Creator post-create contract passed: event-scoped green ratings lead to a fast GPT-4.1-primary grounded quick review with strict deterministic/audit gates and local qwen fallback before independent full upgrades.');
