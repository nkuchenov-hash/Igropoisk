...
if(!reviewDiscovery.includes('discover-review-sources-web-v15.mjs'))fail('Canonical review discovery does not route through v15 historical critic index');
...
if(historicalDiscoveryV15.includes("await import('./discover-review-sources-web-v14.mjs')"))fail('v15 still shares a top-level-await process with the legacy discovery stack');
for(const marker of ["spawnSync('node'","scripts/discover-review-sources-web-v14.mjs","scripts/discover-review-sources-web-v8.mjs",'REVIEW_LEGACY_STACK_TIMEOUT_MS','legacy_stack'])if(!historicalDiscoveryV15.includes(marker))fail(`Isolated legacy-discovery fallback missing: ${marker}`);
...
console.log('Game Creator post-create contract passed: base page, series/rating/media, isolated professional historical critic indexing, non-destructive post-create research, publishable quick review and heavy editorial upgrade are independent and durable.');
