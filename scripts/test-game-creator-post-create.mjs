...
for(const marker of ['build-review-bootstrap-local.mjs','audit-review-bootstrap-local.mjs','grounding_audit?.passed!==true'])if(!commercialQuick.includes(marker))fail(`Commercial quick-review wrapper missing: ${marker}`);
...
if(!enrichmentWorkflow.includes("OPENAI_API_KEY: ''")||!enrichmentWorkflow.includes('qwen3:4b')||!enrichmentWorkflow.includes('qwen3-vl:4b'))fail('Commercial local model routing missing');
for(const marker of ['Ensure required text model for quick review','for attempt in 1 2 3 4 5','Required text model','Ensure optional local vision model for full upgrades','Optional vision model','published quick review remains valid'])if(!enrichmentWorkflow.includes(marker))fail(`Local model resilience missing: ${marker}`);
for(const forbidden of ['models: read','GITHUB_REVIEW_MODEL','GITHUB_VISION_MODEL','scripts/enrich-review-media-github-models.mjs'])if(enrichmentWorkflow.includes(forbidden))fail(`Retired GitHub Models workflow dependency remains: ${forbidden}`);
...
for(const marker of ['build-review-bootstrap-commercial.mjs','POST_CREATE_PUBLISH_PHASE: quick-review','POST_CREATE_PHASE: full-review','POST_CREATE_PUBLISH_PHASE: full-review','scripts/verify-post-create-quick-reviews.mjs','Grounded quick-review publication gate did not pass after bounded retries'])if(!enrichmentWorkflow.includes(marker))fail(`Independent grounded quick/full workflow checkpoint missing: ${marker}`);
...
console.log('Game Creator post-create contract passed: event-scoped green ratings lead to a factual, natural-Russian quick review before independent local full upgrades.');
