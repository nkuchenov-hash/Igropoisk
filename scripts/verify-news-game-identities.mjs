...
const NON_GAME_TITLES=new Set(['twitch','dreamworks','aoc','steam','steam deck','xbox','playstation','nintendo','epic games','valve','ubisoft','electronic arts','ea','activision','blizzard','bethesda','capcom','sega','konami','bandai namco','ign','pc gamer','eurogamer','gamespot','polygon','rock paper shotgun','playground']);
...
const TITLE_SEQUENCE=/\b(?:[A-Z]{2,}|[A-Z][A-Za-z0-9:'’.-]*)(?:\s+(?:(?:of|the|and|for|to|in|on|with)|[A-Z]{2,}|[A-Z][A-Za-z0-9:'’.-]*|\d+)){1,7}\b/g;
...
const rawArticleText=item=>`${text(item,['titleEn','titleRu','title'])} ${text(item,['summaryEn','summaryRu','summary'])}`.trim();
...
function inferredCandidatesFromText(value){
  const seen=new Set();const candidates=[];
  for(const match of String(value||'').matchAll(TITLE_SEQUENCE)){
  ...
  return candidates.sort((a,b)=>a.index-b.index||b.title.length-a.title.length).slice(0,8).map(row=>row.title);
}
function inferredHeadlineCandidates(item){return inferredCandidatesFromText(text(item,['titleEn','titleRu','title'])).slice(0,6)}
function inferredArticleCandidates(item){return inferredCandidatesFromText(rawArticleText(item))}
function hasSpecificEditionContext(item,candidate){
  const hay=articleText(item);const key=normalize(candidate);const index=hay.indexOf(key);if(index<0)return false;
  const window=hay.slice(Math.max(0,index-90),Math.min(hay.length,index+key.length+30));
  return /(?:dlc|expansion|add on|дополнен\p{L}*|аддон\p{L}*|для|for)/iu.test(window);
}
...
let specificGameArticles=0,nonGameArticles=0,ambiguousArticles=0,canonicalMatches=0,verifiedNewGames=0,unsafeHintsRejected=0,headlineInferredGames=0,specificEditionCorrections=0;
...
for(const item of items){
  ...
  for(const hint of hints){
    ...
    if(canonical&&mentioned(item,canonical,hint)){
      ...
      const baseKey=normalize(canonical.title||canonical.slug||'');
      if(baseKey&&baseKey.split(' ').length<=2){
        for(const candidate of inferredArticleCandidates(item)){
          const candidateKey=normalize(candidate);
          if(candidateKey===baseKey||!candidateKey.startsWith(`${baseKey} `)||!hasSpecificEditionContext(item,candidate))continue;
          const exactCanonical=uniqueName(candidate);
          if(exactCanonical){specificGame=canonicalGame(exactCanonical,{title:candidate},item);break}
          let evidence=cache.get(candidateKey);
          if(evidence===undefined){evidence=await verifyExternalTitle(candidate);cache.set(candidateKey,evidence||null)}
          if(evidence){specificGame=externalGame(candidate,evidence,item);verifiedNewGames+=1;break}
        }
      }
      const game=specificGame||canonicalGame(canonical,hint,item);
      if(specificGame)specificEditionCorrections+=1;else canonicalMatches+=1;
...
const report={schema_version:7,generated_at:new Date().toISOString(),provider:'registry-plus-specific-edition-headline-inference-opencritic-wikidata-steam-with-hardware-safety-guards',paid_ai_required:false,articles:normalizedItems.length,specific_game_articles:specificGameArticles,non_game_articles:nonGameArticles,ambiguous_articles:ambiguousArticles,canonical_matches:canonicalMatches,verified_new_game_references:verifiedNewGames,headline_inferred_game_references:headlineInferredGames,specific_edition_corrections:specificEditionCorrections,unsafe_hints_rejected:unsafeHintsRejected,known_person_candidates:knownPersonCandidates.size,unique_games:new Set(normalizedItems.flatMap(item=>(item.games||[]).map(game=>game.gameId))).size,issues};
...
console.log(`[news/game-verifier] ${report.articles} articles; game=${specificGameArticles}; non-game=${nonGameArticles}; ambiguous=${ambiguousArticles}; headline-inferred=${headlineInferredGames}; specific-edition-corrections=${specificEditionCorrections}; unsafe hints rejected=${unsafeHintsRejected}; known person candidates=${knownPersonCandidates.size}; unique games=${report.unique_games}; paid AI required=false.`);
