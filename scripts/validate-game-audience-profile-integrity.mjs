...
for(const file of [
  'scripts/collect-game-audience-evidence.mjs',
  'scripts/build-game-audience-profile.mjs',
  'scripts/lib/game-audience-profile.mjs',
  'scripts/lib/igropoisk-editorial-style.mjs',
  'scripts/lib/game-page-editorial-ai.mjs',
  'scripts/test-game-audience-profile.mjs',
  'docs/IGROPOISK_AUDIENCE_PROFILE.md'
])requireFile(file);
...
requireTokens('scripts/lib/game-page-editorial-ai.mjs',['buildEditorialAudienceContext','INTERNAL AUDIENCE PROFILE','audience_profile_used','audience_profile_confidence','runtime default, not a Game Page']);
...
const test=spawnSync(process.execPath,['scripts/test-game-audience-profile.mjs'],{cwd:root,encoding:'utf8',stdio:'pipe'});
if(test.status!==0)errors.push(`audience profile tests failed: ${(test.stderr||test.stdout||'').slice(-2000)}`);
const result={schema_version:1,module:'game-audience-profile',status:errors.length?'red':'green',checked_at:new Date().toISOString(),fail_open_contract:true,internal_only_contract:true,model_independent_contract:true,errors,test_output:(test.stdout||'').trim().slice(-3000)};
