...
if (!slugOrId) throw new Error('Usage: build-news-game-page-fast <slug-or-id>');
...
if (steamAppId) {
  try {
    const response = await fetch(`https://store.steampowered.com/api/appdetails?appids=${steamAppId}&l=russian&cc=ru`, {
      headers: { 'user-agent': 'IgropoiskNewsGamePage/1.0' },
...
const sourceUrl = first(parser?.source?.url, process.env.NEWS_SOURCE_URL, store, official);
...
const sources = sourceUrl ? [{
  name: parser?.source?.name || (store && sourceUrl === store ? 'Steam Store API' : 'Проверенный источник новости'),
  url: sourceUrl,
...
const publicReady = !released || reviewReady;
...
const game = {
  ...
  publication: {
    ...
    media_green: null,
    mode: 'news_fast_structured_sources',
    updated_at: nowIso,
    gate: {
...
console.log(JSON.stringify({ slug, game_id: entity.id, status: 'green', page_available: true, public_ready: publicReady, review_ready: reviewReady }, null, 2));
