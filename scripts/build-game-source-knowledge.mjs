...
const add=(raw={})=>{
  ...
  byUrl.set(url,{...old,id:old.id||raw.id||`source-${byUrl.size+1}`,name:clean(raw.name||raw.publication||raw.source_name||old.name),title:clean(raw.title||old.title),url,kind:raw.kind||raw.source_kind||old.kind||'source',professional:Boolean(raw.professional??old.professional),roles:[...new Set([...(old.roles||[]),...(raw.roles||[]),...[raw.role,raw.type].map(r=>({structured_fact_source:'facts',professional_review:'review',official_source:'facts',store_source:'description'}[String(r||'')]||String(r||'')).trim()).filter(Boolean)])],semantic:[...new Set([...(old.semantic||[]),...semantic])]});
};
