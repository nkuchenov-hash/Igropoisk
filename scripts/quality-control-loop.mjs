  ...
  if(sources.discovery?.complete!==true){comments=[`Не завершён полный корпус источников: ${sources.counts?.professional_reviews||0} профессиональных материалов.`]}
    ...
    if(rating.status!=='green'||!Number.isFinite(Number(rating.calculation?.score_10)))comments=['Профессиональный рейтинг не прошёл проверку.'];
      ...
      if(knowledgeRun.status!==0||knowledge.status!=='green'||(knowledge.defining_claims||[]).length<4){comments=[(knowledgeRun.stderr||knowledgeRun.stdout||'Не удалось построить знания из источников.').slice(-5000)]}
        ...
        // build-game-page owns its own bounded two-pass editorial repair. QC never calls
        // the same writer or the factual pipeline a second time.
        const editorial=run('editorial-once','scripts/build-game-page.mjs',[slug]);
        if(editorial.status!==0){comments=[(editorial.stderr||editorial.stdout||'Редактор не создал допустимый текст.').slice(-5000)]}
        else{
          normalizeCanonicalTitle();
          const materialized=run('materialize-editorial-once','scripts/materialize-page-editorial.mjs',[slug]);
          if(materialized.status!==0){comments=[(materialized.stderr||materialized.stdout||'Текст не прошёл materialization.').slice(-5000)]}
          else{
            run('content-validation-once','scripts/validate-game-page-content.mjs',[slug]);
            const content=currentContent();
            if(content.status!=='green')comments=[...(content.errors||[])];
            else{
              ...
              if(mediaOk){status='green';comments=[]}else comments=[...(media.comments||[]),'Медиакорпус не прошёл проверку.'];
            }
          }
        }
      }
...
const report={schema_version:15,type:mode,game_slug:slug,game_id:gameId||null,canonical_title:canonicalTitle||null,checked_at:new Date().toISOString(),status,green:status==='green',comments,revision_history:history,policy:{page_qc_architecture:'single factual build + writer-owned bounded repair',page_sources_rebuilt_per_editorial_retry:false,page_rating_rebuilt_per_editorial_retry:false,page_knowledge_rebuilt_per_editorial_retry:false,page_media_rebuilt_when_already_green:false,page_editorial_writer_invocations:1,page_ready_requires_source_corpus:true,page_ready_requires_source_grounded_knowledge:true,page_ready_requires_content_validation:true,page_ready_requires_media_validation:true,page_ready_requires_review_article:false,review_subsystem_separate:true,page_editorial_ai_provider:'local-qwen-ollama'}};
