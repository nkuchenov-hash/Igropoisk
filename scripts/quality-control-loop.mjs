...
for(let attempt=1;attempt<=attempts;attempt++){
  if(mode==='page'){
    ...
    if(process.env.OPENAI_API_KEY){const child=run(`page-revision-${attempt}`,'scripts/build-game-page.mjs',[slug]);if(child.status!==0)comments=[...comments,(child.stderr||'').slice(-3000)]}
  }else{
    ...
    const reviews=currentReviews(),rating=currentRating();
    const corpusGreen=reviews.publication_gate?.status==='green';const ratingGreen=rating.status==='green';
    ...
    const steps=[['media-discovery','scripts/discover-review-media.mjs'],['synthesis','scripts/synthesize-review.mjs'],['media-enrichment','scripts/enrich-review-media.mjs'],['validation','scripts/validate-review-output.mjs']];
    ...
    const validation=currentReviewOutput();if(technicalOk&&validation?.passed===true){status='green';comments=[];break}
    status='red-needs-revision';if(validation?.errors?.length)comments=validation.errors;
  }
}
const report={schema_version:1,type:mode,game_slug:slug,game_id:gameId||null,checked_at:new Date().toISOString(),status,green:status==='green',comments,revision_history:history,policy:{quality_never_terminally_blocks:true,red_means_revise_again:true,immediate_revision_attempts:attempts,keep_queued_until_green:true}};
...
console.log(JSON.stringify({mode,slug,status,attempts:history.length,comments:comments.slice(0,8)},null,2));
process.exit(0);
