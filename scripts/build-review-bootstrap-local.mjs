...
const QUICK_REVIEW_TIMEOUT_MS=Math.max(120000,Math.min(600000,Number(process.env.QUICK_REVIEW_TIMEOUT_MS||300000)));
...
async function localJson(prompt,temperature=0.2){return chatJson({system:'Ты опытный русскоязычный игровой журналист. Пиши живой оригинальный текст, без повторов, строго опираясь на предоставленные профессиональные рецензии.',prompt,schema,temperature,numCtx:QUICK_REVIEW_NUM_CTX,numPredict:QUICK_REVIEW_NUM_PREDICT,timeoutMs:QUICK_REVIEW_TIMEOUT_MS})}
async function generateQuickReview(){
  const failures=[];
  for(let attempt=1;attempt<=2;attempt++){
    try{
      const retry=attempt>1?`${basePrompt}\n\nПредыдущий вариант не прошёл редакционный gate. Перепиши материал целиком: сделай тезисы конкретнее, устрани повторы и шаблонные заголовки.`:basePrompt;
      const generated=await localJson(retry,attempt>1?0.15:0.2),quality=assessEditorialQuality(generated);
      if(quality.passed)return{generated,quality,provider:'local-ollama',model:LOCAL_EDITORIAL_MODEL,attempt,failures};
      failures.push(`local attempt ${attempt}: ${quality.reasons.join(', ')}`);
    }catch(error){failures.push(`local attempt ${attempt}: ${error?.message||String(error)}`)}
  }
...
write(`data/parser-runs/review-bootstrap-${slug}.json`,{parser:'review-bootstrap-quality-v3-local',status:'green',game_slug:slug,checked_at:now,score,sources:article.sources.length,words,sections:sections.length,provider:generatedResult.provider,model:generatedResult.model,generation_attempt:generatedResult.attempt,editorial_quality:generatedResult.quality});
