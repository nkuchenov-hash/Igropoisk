...
if(!discovered){const run=spawnSync(process.execPath,[path.join(root,'scripts/discover-game-sources-web.mjs'),slug],{cwd:root,env:process.env,encoding:'utf8',stdio:['ignore','pipe','pipe'],maxBuffer:24*1024*1024});webRun={status:run.status,cached:false};discovered=read(`data/research/${slug}-independent-web-sources.json`,null)}
let publicationHubs=read(`data/research/${slug}-publication-hubs.json`,null),hubRun={status:0,cached:Boolean(publicationHubs)};
const hubScriptExists=fs.existsSync(path.join(root,'scripts/discover-game-publication-hubs.mjs'));
if(!publicationHubs&&hubScriptExists){const run=spawnSync(process.execPath,[path.join(root,'scripts/discover-game-publication-hubs.mjs'),slug],{cwd:root,env:process.env,encoding:'utf8',stdio:['ignore','pipe','pipe'],maxBuffer:8*1024*1024});hubRun={status:run.status,cached:false};publicationHubs=read(`data/research/${slug}-publication-hubs.json`,null)}
...
const professionalCount=pubs.size,scoredCount=[...pubs.values()].filter(x=>x.score_eligible).length;
const coveragePassed=professionalCount>=minimumProfessional&&scoredCount>=minimumScored;
const webDiscoveryComplete=Boolean(discovered&&typeof discovered==='object'&&Array.isArray(discovered.sources));
const publicationHubDiscoveryComplete=!hubScriptExists||Boolean(publicationHubs&&typeof publicationHubs==='object'&&Array.isArray(publicationHubs.sources));
const scanComplete=webDiscoveryComplete&&publicationHubDiscoveryComplete;
const output={schema_version:10,game_slug:slug,game_id:draft.game_id||reviews.game_id||null,title:draft.identity.title,generated_at:checkedAt,ownership:'game-page-module',purpose:'Canonical reusable evidence corpus for game page, Game DNA, media, descriptions, rating and editorial review.',discovery:{method:'multi-engine-direct-review-discovery-plus-publication-hubs',complete:scanComplete,coverage_passed:coveragePassed,coverage_blocks_page_publication:false,minimum_professional_sources:minimumProfessional,minimum_scored_sources:minimumScored,professional_minimum_passed:professionalCount>=minimumProfessional,scored_minimum_passed:scoredCount>=minimumScored,web_search_cached:webRun.cached,web_discovery_exit_code:webRun.status,publication_hubs_cached:hubRun.cached,publication_hubs_exit_code:hubRun.status,publication_hub_sources:publicationHubs?.sources?.length||0,providers:discovered?.providers||{},query_count:discovered?.queries||0,candidate_count:discovered?.candidates||0,accepted_count:discovered?.accepted||0,distinct_publications:professionalCount,ai_required:false,direct_review_only_for_professional_count:true},counts:{total:sources.length,scored:scoredCount,professional_reviews:professionalCount,editorial_sources:sources.filter(x=>x.kind==='editorial-source').length},sources};
write(`data/game-sources/${slug}.json`,output);write(`data/parser-runs/game-sources-${slug}.json`,{parser:'game-source-corpus',status:scanComplete?'green':'needs_revision',game_slug:slug,checked_at:checkedAt,total_sources:sources.length,scored_sources:scoredCount,professional_reviews:professionalCount,minimum_professional_sources:minimumProfessional,minimum_scored_sources:minimumScored,coverage_passed:coveragePassed,coverage_blocks_page_publication:false,web_search_cached:webRun.cached,web_discovery_exit_code:webRun.status,publication_hubs_exit_code:hubRun.status,scan_complete:scanComplete,output:`data/game-sources/${slug}.json`});
console.log(JSON.stringify({slug,status:scanComplete?'green':'needs_revision',total:sources.length,professional_reviews:professionalCount,scored:scoredCount,coverage_passed:coveragePassed,coverage_blocks_page_publication:false,editorial_sources:output.counts.editorial_sources,publication_hub_sources:publicationHubs?.sources?.length||0,scan_complete:scanComplete,web_search_cached:webRun.cached,web_discovery_ms:discovered?.elapsed_ms??null},null,2));if(!scanComplete)process.exitCode=2;
