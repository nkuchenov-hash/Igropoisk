...
function add(raw={}){
 ...
 const professional=Boolean(raw.professional??existing?.professional??rawKind==='professional-review');
 const editorial=['editorial','review','retrospective_review','opinion','longread','professional-review'].includes(rawKind);
 ...
 const next={id:existing?.id||`source-${seen.size+1}`,name:String(raw.name||raw.publication||raw.source_name||raw.source||existing?.name||host(url)||'Источник'),title:String(raw.title||existing?.title||''),url,domain:host(url),kind,professional,roles,score:hasScore?Number(raw.score):(existing?.score??null),scale:hasScore?Number(raw.scale):(existing?.scale??null),grade:professional?String(raw.grade||existing?.grade||''):'',score_eligible:Boolean(professional&&(raw.score_eligible??existing?.score_eligible??false)),checked_at:String(raw.checked_at||raw.validation?.checked_at||existing?.checked_at||checkedAt),provenance:String(raw.provenance||existing?.provenance||'game-page-source-corpus')};
 if(professional&&Number.isFinite(next.score)&&Number.isFinite(next.scale)&&next.scale>0){next.score_eligible=true;next.normalized_10=Number((next.score/next.scale*10).toFixed(3))}else if(professional&&next.grade)next.score_eligible=true;else if(!professional){next.score=null;next.scale=null;next.grade='';next.score_eligible=false}
 seen.set(key,next)
}
...
const output={schema_version:7,game_slug:slug,game_id:draft.game_id||reviews.game_id||null,title:draft.identity.title,generated_at:checkedAt,ownership:'game-page-module',purpose:'Canonical reusable evidence corpus for game page, Game DNA, media, descriptions, rating and editorial review.',discovery:{method:'multi-engine-direct-review-discovery',complete:scanComplete,minimum_professional_sources:minimumProfessional,minimum_scored_sources:minimumScored,professional_minimum_passed:professionalCount>=minimumProfessional,scored_minimum_passed:scoredCount>=minimumScored,web_search_cached:webRun.cached,web_discovery_exit_code:webRun.status,providers:discovered?.providers||{},query_count:discovered?.queries||0,candidate_count:discovered?.candidates||0,accepted_count:discovered?.accepted||0,distinct_publications:professionalCount,ai_required:false},counts:{total:sources.length,scored:scoredCount,professional_reviews:professionalCount,editorial_sources:sources.filter(x=>x.kind==='editorial-source').length},sources};
...
console.log(JSON.stringify({slug,status:scanComplete?'green':'needs_revision',total:sources.length,professional_reviews:professionalCount,scored:scoredCount,editorial_sources:output.counts.editorial_sources,scan_complete:scanComplete,web_search_cached:webRun.cached,web_discovery_ms:discovered?.elapsed_ms??null},null,2));if(!scanComplete)process.exitCode=2;
