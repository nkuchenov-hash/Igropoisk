...
function add(raw={}){
  ...
  const next={id:existing?.id||`source-${seen.size+1}`,name:String(raw.name||raw.publication||raw.source_name||raw.source||existing?.name||host(url)||'Источник'),title:String(raw.title||existing?.title||''),url,domain:host(url),kind,roles,score:hasScore?Number(raw.score):(existing?.score??null),scale:hasScore?Number(raw.scale):(existing?.scale??null),grade:String(raw.grade||existing?.grade||''),score_eligible:Boolean(raw.score_eligible??existing?.score_eligible??false),checked_at:String(raw.checked_at||raw.validation?.checked_at||existing?.checked_at||checkedAt),provenance:String(raw.provenance||existing?.provenance||'game-page-source-corpus')};
...
const professional=sources.filter(item=>item.kind==='professional-review');
const publicationMap=new Map();
for(const item of professional){const key=item.domain||item.name.toLowerCase();const old=publicationMap.get(key);if(!old||(!old.score_eligible&&item.score_eligible))publicationMap.set(key,item)}
const professionalCount=publicationMap.size;
const scoredCount=[...publicationMap.values()].filter(item=>item.score_eligible).length;
...
const output={schema_version:6,game_slug:slug,game_id:draft.game_id||reviews.game_id||null,title:draft.identity.title,generated_at:checkedAt,ownership:'game-page-module',purpose:'Canonical reusable evidence corpus for game page, Game DNA, media, descriptions, rating and editorial review.',discovery:{method:'broad-parallel-web-search-plus-live-verification',complete:scanComplete,minimum_professional_sources:minimumProfessional,minimum_scored_sources:minimumScored,professional_minimum_passed:corpusMinimumPassed,scored_minimum_passed:scoreMinimumPassed,web_search_cached:webRun.cached,web_discovery_exit_code:webRun.status,providers:discovered?.providers||{},query_count:discovered?.queries||0,candidate_count:discovered?.candidates||0,accepted_count:discovered?.accepted||0,distinct_publications:professionalCount,ai_required:false},counts:{total:sources.length,scored:scoredCount,professional_reviews:professionalCount},sources};
