...
const SYNTHESIS_ARCHITECTURE='paragraphwise-persistent-v4-small-writer-final-4b-audit';
...
async function generateSectionContinuation(index,section,pass,reason=''){
  ...
  return chatJson({system:'Ты старший русскоязычный игровой журналист. Добавь ровно один новый абзац по evidence. Не повторяй существующий текст и не добавляй фактов вне evidence.',prompt:`Раздел ${index+1}/${targetSections} обзора ${identity.title}. Фокус: ${theme.focus}. Сейчас ${currentWords} слов. Напиши один новый абзац 95–125 русских слов, раскрывающий ДРУГУЮ конкретную грань этого фокуса. Английские слова запрещены кроме Fallout/RPG.\nПОСЛЕДНИЕ АБЗАЦЫ:${JSON.stringify(existing)}\nSOURCE IDS:${ids.join(', ')}\nCOMPACT EVIDENCE:${JSON.stringify(evidence)}${reason?`\nЦЕЛЬ:${compactText(reason,220)}`:''}`,schema:sectionContinuationSchema,temperature:0.12,numCtx:LOCAL_SECTION_CONTINUATION_NUM_CTX,numPredict:LOCAL_SECTION_CONTINUATION_NUM_PREDICT,timeoutMs:LOCAL_PARAGRAPH_TIMEOUT_MS,model:LOCAL_WRITER_MODEL})
}
function paragraphValid(paragraph,current=[]){const wc=countWords(paragraph);return wc>=55&&wc<=170&&!lowerLatin(paragraph).length&&!current.some(existing=>nearDuplicateText(existing,paragraph))}
...
async function topUpSection(index,section,{targetWords=SECTION_TARGET_WORDS,reason=''}={}){
  ...
  while((countWords((current.paragraphs||[]).join(' '))<targetWords||(current.paragraphs||[]).length<minParagraphs)&&(current.paragraphs||[]).length<MAX_PARAGRAPHS_PER_SECTION&&stalls<MAX_PARAGRAPH_STALLS){
    pass++;const before=countWords((current.paragraphs||[]).join(' '));current=await appendParagraph(index,current,pass,reason);const after=countWords((current.paragraphs||[]).join(' '));if(after<=before){stalls++;continue}stalls=0;state.sections[id]=current;persist();
  }
  return current;
}
