...
require(integrated.length>=280,'integrated_description is too short');require(cyrillicRatio(integrated)>=0.55,'integrated_description is not natural Russian');
require(campaign.length>=120,'campaign is too short');require(cyrillicRatio(campaign)>=0.55,'campaign is not natural Russian');
...
for(const rx of forbidden){require(!rx.test(short),`generic short_description: ${rx}`);require(!rx.test(integrated),`generic integrated_description: ${rx}`);require(!rx.test(campaign),`generic campaign: ${rx}`)}
...
const out={schema_version:1,game_slug:slug,title:clean(draft.identity.title),release_year:releaseYear,developer,genres,short_description:short,integrated_description:integrated,campaign,features,source_corpus:`data/game-sources/${slug}.json`,source_count:Number(corpus?.counts?.total||corpus?.sources?.length||0),generated_at:new Date().toISOString(),quality_status:'green'};
write(`data/page-editorial/${slug}.json`,out);
console.log(JSON.stringify({slug,status:'green',output:`data/page-editorial/${slug}.json`},null,2));
