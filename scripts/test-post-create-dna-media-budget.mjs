...
for(const marker of ['natural_russian','translationese_absence','factual_grounding','unsupportedNumbers','lowercase_latin_intrusions','grounding_audit','process.exit(2)'])if(!quickAudit.includes(marker))fail(`Factual/language quick-review audit missing: ${marker}`);
...
for(const marker of ['scripts/build-review-bootstrap-commercial.mjs','scripts/verify-post-create-quick-reviews.mjs','for attempt in 1 2 3','Grounded quick-review publication gate did not pass after bounded retries'])if(!workflow.includes(marker))fail(`Fail-closed grounded quick-review retry contract missing: ${marker}`);
...
console.log('Post-create event-scoped execution, bounded checkout, semantic-precision DNA, factual/natural-Russian fail-closed quick review, local-only full upgrade, fair queue and bounded media contract passed.');
