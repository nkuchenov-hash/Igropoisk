...
const quickAt=workflow.indexOf('Build bounded quick reviews for green-rated event-scoped games'),visionAt=workflow.indexOf('Ensure optional local vision model for full upgrades'),fullAt=workflow.indexOf('Run bounded full-review upgrades for event-scoped games');
if(quickAt<0||visionAt<quickAt||fullAt<visionAt)fail('Vision model provisioning must happen after quick-review publication and before the full-review upgrade');
console.log('Post-create event-scoped execution, bounded checkout, semantic-precision DNA, durable local quick review, local-only full upgrade, fair queue and bounded media contract passed.');
