...
for(const marker of ['300–420','minItems:4,maxItems:4','paragraphs||[]).length!==2','canonical_score_eligible','usedIds.size<3','numCtx:6144','numPredict:2400','commercial_depth_gate:true'])if(!commercialSynthesis.includes(marker))fail(`Reliable grounded commercial synthesis missing: ${marker}`);
for(const marker of ['natural_russian','translationese_absence','factual_grounding','unsupportedNumbers','lowercase_latin_intrusions','citedIds','citedPublications','numCtx:8192','numPredict:900',"passed:{type:'boolean'}",'quality_problems','!evidenceText.includes(value)','grounding_audit','process.exit(2)'])if(!quickAudit.includes(marker))fail(`Explicit factual/language quick-review audit missing: ${marker}`);
...
console.log('Post-create event-scoped execution, bounded checkout, semantic-precision DNA, reliable 4x2 commercial synthesis, explicit factual/natural-Russian audit, local-only full upgrade, fair queue and bounded media contract passed.');
