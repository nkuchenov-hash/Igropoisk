...
for(const marker of ['scripts/build-game-dna.mjs','scripts/validate-game-dna.mjs','scripts/build-similarity-index.mjs','data/parser-runs/game-post-create-dna.json','missingDnaSlugs','game/${slug}/index.html',"game_dna:'ready'","dna:'ready'",'validation_scope:\'per_game_targeted\'',"run('scripts/validate-game-dna.mjs',[slug])",'publication_policy:\'valid_dna_publishes_even_if_other_games_or_similarity_need_revision\'','ready_slugs:validDnaSlugs'])if(!dna.includes(marker))fail(`Post-create DNA materializer missing: ${marker}`);
if(dna.includes('process.exitCode=2'))fail('Independent DNA/similarity failures can still block the valid DNA checkpoint');
for(const marker of ['const requested = new Set(process.argv.slice(2)','requested.size ? allFiles.filter','requested DNA entity is missing',"requested.size ? 'targeted' : 'catalog'"])if(!validator.includes(marker))fail(`Targeted Game DNA validation missing: ${marker}`);
...
console.log('Post-create partial-progress DNA and bounded media contract passed.');
