...
for(const marker of ['260–360','canonical_score_eligible','usedIds.size<3','numCtx:6144','numPredict:2200','commercial_depth_gate:true'])if(!commercialSynthesis.includes(marker))fail(`Direct grounded commercial synthesis missing: ${marker}`);
for(const marker of ['natural_russian','translationese_absence','factual_grounding','unsupportedNumbers','lowercase_latin_intrusions','citedIds','citedPublications','numCtx:8192','numPredict:1000','grounding_audit','process.exit(2)'])if(!quickAudit.includes(marker))fail(`Compact factual/language quick-review audit missing: ${marker}`);
...
console.log('Post-create event-scoped execution, bounded checkout, semantic-precision DNA, direct two-pass factual/natural-Russian quick review, local-only full upgrade, fair queue and bounded media contract passed.');
