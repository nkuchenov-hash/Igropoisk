...
const postCreateWorkflow=read('.github/workflows/game-post-create-enrichment.yml');
const creator=read('scripts/ensure-game-page.mjs');
const postCreate=read('scripts/run-game-post-create-enrichment.mjs');
...
need(news,'news-game-page-fast.yml','news must dispatch the canonical fast Game Creator');
need(news,'shared Game Creator','news must identify the shared Game Creator as its page-creation boundary');
need(creator,'data/game-enrichment-requests/${slug}.json','Game Creator must queue non-blocking post-create enrichment');
need(creator,"reviewNeeded = released && !reviewReady",'released games without a canonical review must automatically queue editorial enrichment');
need(postCreateWorkflow,"'data/game-enrichment-requests/*.json'",'post-create enrichment requests must trigger immediately on staging');
need(postCreateWorkflow,'run-game-post-create-enrichment.mjs','post-create workflow must dispatch the shared enrichment runner');
need(postCreate,'calculate-ratings-from-research.mjs','post-create enrichment must bootstrap the review-derived rating before full editorial completion');
need(postCreate,'quality-control-loop.mjs','post-create enrichment must reuse the canonical full review quality loop');
...
need(publisher,'full review QC is not green','production promotion must require full review QC, not only corpus/score');
need(publisher,'review article is not published/green','production promotion must require a published green review article');
need(publisher,'published review article HTML is missing','production promotion must require rendered review HTML');
need(publisher,'does not match canonical','production promotion must reject review/article score divergence');
