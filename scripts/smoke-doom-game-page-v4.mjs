...
try{
  ...
  await page.waitForFunction(()=>document.querySelectorAll('#similarGames [data-similarity-score]').length>=1&&document.querySelector('#editorialScore')?.textContent?.trim()==='8.7',{timeout:15000});
  ...
  const state=await page.evaluate(()=>({
    ...
    editorialScore:document.querySelector('#editorialScore')?.textContent?.trim()||'',
    ...
    featuredMeta:document.querySelector('#featuredReview .ig-review-feature__meta')?.textContent?.replace(/\s+/g,' ').trim()||'',
  ...
  if(state.editorialScore!=='8.7')errors.push(`Canonical DOOM rating is wrong: ${state.editorialScore}`);
  if(state.reviewRows!==0)errors.push(`Ungated professional review rows are visible: ${state.reviewRows}`);
  if(state.featuredScore!=='—')errors.push(`Review feature must remain gated until professional publication is green: ${state.featuredScore}`);
  if(!/Профессиональные оценки собираются и проверяются/i.test(state.featuredMeta))errors.push(`Review publication-gate message is missing: ${state.featuredMeta}`);
