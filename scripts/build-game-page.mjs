...
required(result.identity.title,'identity.title');required(result.release.date_text||result.release.date||result.release.status,'release');required(result.companies.developers,'companies.developers');required(result.companies.publishers,'companies.publishers');required(result.classification.genres,'classification.genres');required(result.classification.platforms,'classification.platforms');required(result.editorial.short_description,'editorial.short_description');required(result.editorial.integrated_description,'editorial.integrated_description');required(result.editorial.campaign,'editorial.campaign');required(result.media.hero,'media.hero');required(result.media.cover,'media.cover');
...
const publication={status:passed?'published':'review',gate_passed:passed,updated_at:checkedAt,gate:{minimum_sources:Number(gate.minimum_sources||10),accepted_sources:result.sources.length,minimum_screenshots:Number(gate.minimum_screenshots||6),accepted_screenshots:result.media.screenshots.length,identity_sources:identitySources.length,missing,passed}};
const game={schema_version:3,publication,...result,updated_at:checkedAt};
writeJSON(`data/drafts/${slug}.json`,game);
writeJSON(`data/parser-runs/game-page-${slug}.json`,{parser:'game-page-builder',status:passed?'success':'blocked',game_slug:slug,checked_at:checkedAt,gate:publication.gate,output:passed?'data/game-content':`data/drafts/${slug}.json`});
if(!passed){console.error(`Page gate failed for ${slug}: ${missing.join(', ')}`);process.exit(2)}
...
chunkData.schema_version=Math.max(Number(chunkData.schema_version||1),3);
...
const entry={title:result.identity.title,year,slug};
...
console.log(JSON.stringify({slug,passed,year,chunk,sources:result.sources.length,screenshots:result.media.screenshots.length},null,2));
