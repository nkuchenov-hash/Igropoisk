...
const cyrillicRatio=value=>{const s=plain(value);const letters=(s.match(/[A-Za-zА-Яа-яЁё]/g)||[]).length;return letters?(s.match(/[А-Яа-яЁё]/g)||[]).length/letters:0};
...
const prompt=`Собери полностью проверенную карточку игры для публичной русскоязычной страницы Игропоиска. Используй активный веб-поиск. Речь должна идти только о точной игре и точной версии, указанной ниже. Не смешивай ремейк, ремастер, продолжение, DLC или другую игру с похожим названием.\n\nИСХОДНЫЕ ДАННЫЕ ИЗ КАНОНИЧЕСКОГО ЧЕРНОВИКА/STEAM:\n${JSON.stringify(seed,null,2)}\n\nОБЯЗАТЕЛЬНО:\n- Подтверди идентичность минимум официальным источником и крупной базой/магазином либо двумя независимыми базами.\n- release.date и release.date_text — дата ПЕРВОГО ОРИГИНАЛЬНОГО КОММЕРЧЕСКОГО РЕЛИЗА этой игры, а не дата её более позднего появления в Steam/GOG, переиздания или патча. Если канонический черновик содержит более раннюю дату, проверь её и не заменяй датой Steam-листинга без доказательства ошибки.\n- Собери не менее ${Number(gate.minimum_sources||10)} реально открываемых прямых источников. Каталоги-разделы и поисковые выдачи не подходят.\n- Для дат, компаний, платформ и системных требований приоритет у официальных страниц, архивных официальных материалов, магазинов и надёжных баз.\n- title — точное каноническое пользовательское название игры. Никогда не используй slug, техническое имя или имя файла как title.\n- ВСЕ поля editorial и подписи media должны быть написаны естественным русским языком. Не копируй и не оставляй английский Steam-текст.\n- short_description — минимум 90 символов, живое краткое описание сути игры.\n- integrated_description — минимум 280 символов, содержательно объясняет жанр, игровой цикл, структуру мира, ключевые механики и чем игра выделяется.\n- campaign — минимум 120 символов: объясняет сюжетную кампанию, её структуру и роль игрока либо прямо и содержательно объясняет отсутствие кампании.\n- Нужны минимум ${Number(gate.minimum_features||4)} конкретные особенности, каждая не короче полноценной осмысленной фразы.\n- hero — только качественный официальный key art / promotional artwork игры, НЕ gameplay screenshot. cover — отдельная вертикальная официальная обложка. Скриншоты идут отдельно после арта.\n- Нужны hero, cover и минимум ${Number(gate.minimum_screenshots||6)} полноразмерных реальных кадров именно этой игры. Не используй логотипы, миниатюры, фан-арт, растянутые/размытые/пережатые картинки и кадры другой версии.\n- artwork должен содержать официальный арт с прямым source_url; не подменяй artwork скриншотом.\n- Если существует официальный трейлер или видео, обязательно добавь его и поставь official_video_exists=true.\n- Каждому изображению и видео укажи source_url.\n- steam_appid сохраняй из исходных данных; не угадывай другой ID.\n- checked_at для всех источников: ${checkedAt}.\n- slug должен быть ${slug}.`;
...
if(plain(result.editorial.short_description).length<90||cyrillicRatio(result.editorial.short_description)<0.55)missing.push('editorial.short_description_quality');
if(plain(result.editorial.integrated_description).length<280||cyrillicRatio(result.editorial.integrated_description)<0.55)missing.push('editorial.integrated_description_quality');
if(plain(result.editorial.campaign).length<120||cyrillicRatio(result.editorial.campaign)<0.55)missing.push('editorial.campaign_quality');
if((result.editorial.features||[]).length<Number(gate.minimum_features||4)||result.editorial.features.some(item=>plain(item).length<18))missing.push(`features:${result.editorial.features?.length||0}`);
