...
const {data:editorial,provider,model}=await generateFreeEditorialJSON({system:'Ты русскоязычный игровой редактор. Пиши конкретно, естественно и фактологично. Возвращай только валидный JSON.',prompt,temperature:0.25});
const features=Array.isArray(editorial?.features)?editorial.features.map(x=>String(x||'').trim()).filter(Boolean):[];
const next={short_description:String(editorial?.short_description||'').trim(),integrated_description:String(editorial?.integrated_description||'').trim(),campaign:String(editorial?.campaign||'').trim(),features};
...
draft.editorial={...(draft.editorial||{}),...next};draft.publication={...(draft.publication||{}),status:'needs_revision',public_ready:false,quality_status:'editorial_generated_pending_qc'};draft.updated_at=new Date().toISOString();
