...
const evidence=(review.reviews||[]).map(s=>({id:s.id,publication:s.publication,language:s.language||'',evidence_points:(s.evidence_points||[]).slice(0,2),praise:(s.praise||[]).slice(0,1),criticism:(s.criticism||[]).slice(0,1),mechanics:(s.mechanics||[]).slice(0,2)}));
...
const prompt=`Оцени качество готового русскоязычного игрового обзора как строгий выпускающий редактор. Проверь естественность русского языка, цельность авторской подачи, отсутствие буквальных переводных конструкций, конкретность и опору утверждений на source_ids/evidence. Высокие значения допустимы только для текста, который звучит как оригинальная работа опытного русскоязычного игрового журналиста. Evidence ниже — компактная выборка из уже верифицированного канонического корпуса; оцени именно качество текста и обоснованность его source_ids, не требуй полного пересказа всех источников. Верни только JSON по заданной схеме.\n\nТЕКСТ:\n${JSON.stringify(prose)}\n\nEVIDENCE:\n${JSON.stringify(evidence)}`;
const scores=await chatJson({system:'Ты строгий выпускающий редактор русскоязычного игрового издания. Не завышай оценки качества.',prompt,schema,temperature:0.1,numCtx:16384,numPredict:1200,timeoutMs:420000});
const passed=Object.entries(thresholds).every(([k,v])=>Number(scores[k])>=v),report={schema_version:3,game_slug:slug,checked_at:new Date().toISOString(),passed,provider:'local-ollama',model:LOCAL_EDITORIAL_MODEL,article_prose_sha256:proseHash,thresholds,scores,evidence_compaction:{points_per_source:2,praise_per_source:1,criticism_per_source:1,mechanics_per_source:2,num_ctx:16384,num_predict:1200}};write(`data/parser-runs/review-language-${slug}.json`,report);console.log(JSON.stringify({slug,passed,model:LOCAL_EDITORIAL_MODEL,scores},null,2));if(!passed)process.exitCode=2;
