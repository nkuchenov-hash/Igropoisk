...
const controlSlugs=new Set(['baldurs-gate-3','doom']);
...
function similarityState(similarity){
  const recommendations=Array.isArray(similarity?.recommendations)?similarity.recommendations:[];
  const profileNeedsEnrichment=Boolean(similarity?.profile_quality?.needs_enrichment);
  const populatedAxes=Number(similarity?.profile_quality?.populated_axes||0);
  return{recommendations:recommendations.length,populatedAxes,profileNeedsEnrichment,green:recommendations.length>0&&!profileNeedsEnrichment};
}
...
for(const game of catalog){
  ...
  const relationChecked=Boolean(draft?.relations?.checked_at);const relationGreen=relationChecked&&!similarityQuality.profileNeedsEnrichment;const reviewGreen=review?.publication_gate?.status==='green'&&Number(review?.publication_gate?.accepted||review?.reviews?.length||0)>=10;const ratingGreen=rating?.status==='green'&&Number.isFinite(Number(rating?.calculation?.score_10));const guidesGreen=guides?.status==='green'&&Number(guides?.accepted||guides?.guides?.length||0)>=4;
  queue.games[slug]={game_id:game.game_id||'',title:game.title||slug,technical:technical.green?'green':technical.appid?'needs_revision':'not_applicable',relations:relationGreen?'green':'needs_revision',reviews:reviewGreen&&ratingGreen?'green':'needs_revision',guides:guidesGreen?'green':'needs_revision',similarity:similarityQuality.green?'green':'needs_revision',similarity_profile_axes:similarityQuality.populatedAxes,similarity_recommendations:similarityQuality.recommendations};
