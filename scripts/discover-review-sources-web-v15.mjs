...
let fallback=null;
if(!legacyUsable){
  fallback=runLegacy('scripts/discover-review-sources-web-v8.mjs');
  matrixAfterLegacy=read(`data/research/${slug}-source-matrix.json`,null);
}
...
if(!historical){
  const finalStatus=Number(fallback?.status??legacy.status);
  if(![0,2].includes(finalStatus))process.exitCode=2;
  process.exit();
}
...
async function fetchCriticIndex(){
  try{
    const response=await fetch(indexUrl,{redirect:'follow',signal:AbortSignal.timeout(10000),headers:{'user-agent':'Mozilla/5.0 (compatible; IgropoiskHistoricalCriticIndex/15.1)','accept-language':'en-US,en;q=.9'}});
...
const run=read(`data/parser-runs/review-web-discovery-${slug}.json`,{});write(`data/parser-runs/review-web-discovery-${slug}.json`,{...run,parser:'review-professional-historical-index-v15.1',status:result.green?'green':'needs_revision',checked_at:checkedAt,accepted:result.accepted.length,scored:result.scored,professional_only:true,legacy_stack:{primary:legacy,fallback},historical_critic_index:{provider:'metacritic',url:indexUrl,reachable:indexResult.reachable,status:indexResult.status||0,rows:indexResult.rows.length,added_sources:result.indexedAdded,added_scores:result.indexedScoresAdded,metascore_as_vote:false,user_scores_as_votes:false},removed_user_generated:result.removedUserGenerated});
console.log(JSON.stringify({slug,historical:true,professional_only:true,legacy_status:legacy.status,fallback_status:fallback?.status??null,accepted:result.accepted.length,scored:result.scored,metacritic_rows:indexResult.rows.length,indexed_sources_added:result.indexedAdded,indexed_scores_added:result.indexedScoresAdded,removed_user_generated:result.removedUserGenerated,status:result.green?'green':'red-needs-revision'},null,2));
