...
// A page main-thread deadlock must fail this strict smoke quickly enough to preserve the
// last useful phase/slug in CI logs. Assertions and navigation timeouts remain unchanged.
const browser=await puppeteer.launch({executablePath,headless:true,protocolTimeout:45000,args:['--no-sandbox','--disable-dev-shm-usage']});
...
const diagnostics=[];
const phase=(slug,value)=>console.log(JSON.stringify({browser_smoke:{slug,phase:value,at:new Date().toISOString()}}));
try{
  for(const [slug,appid] of games){
    phase(slug,'game-open');
  ...
  phase('3-japan-stigmatized-property','game-open');
  ...
  phase('3-japan-stigmatized-property','game-evaluate');
