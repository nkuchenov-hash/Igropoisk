...
try{
  ...
  const japanBrowser=await launchBrowser();
  let japanState;
  try{
    const japan=await japanBrowser.newPage();await japan.setViewport({width:1440,height:1000});await japan.goto(`http://127.0.0.1:4175/game/3-japan-stigmatized-property/?smoke=${Date.now()}`,{waitUntil:'domcontentloaded',timeout:30000});try{await japan.waitForFunction(()=>document.querySelector('#gameTitle')?.textContent?.trim(),{timeout:10000})}catch{};
    japanState=await japan.evaluate(()=>({title:document.querySelector('#gameTitle')?.textContent||'',details:document.querySelector('#details')?.textContent||'',bodyStart:document.body.textContent.trim().slice(0,220)}));
