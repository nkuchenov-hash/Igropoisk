...
async function fetchText(url,locale){const response=await fetch(url,{signal:AbortSignal.timeout(18000),headers:{'user-agent':'Mozilla/5.0 IgropoiskReleaseMediaPanel/1.0','accept-language':locale==='ru'?'ru-RU,ru;q=0.9,en;q=0.6':'en-US,en;q=0.9'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);return response.text()}
...
const releases=(releasesDoc.releases||[]).map(item=>({...item,editorial_quality:{...(item.editorial_quality||{})}}));
...
for(const game of releases){const popular=popularMatch(game);const previousNames=[...(game.editorial_quality?.media_publishers||[]),...(game.anticipation?.independent_publishers||[]),...(popular?.news_publishers||[])];const fresh=researched.get(game.id);const intersection=buildMediaIntersection({publisherNames:previousNames,evidence:fresh?.evidence||[],config,generatedAt:checkedAt});if(fresh?.failed===2&&game.media_intersection?.overall_count>intersection.overall_count){game.media_intersection={...game.media_intersection,research_status:'stale-fallback',generated_at:checkedAt}}else{game.media_intersection={...intersection,research_status:fresh?(fresh.failed?'partial':'fresh'):'not-prioritized'}}game.editorial_quality={...(game.editorial_quality||{}),media_publishers:game.media_intersection.publishers||[],media_publisher_families:game.media_intersection.publisher_families||[],media_intersection_count:Number(game.media_intersection.overall_count||0),media_region_counts:game.media_intersection.region_counts||{}};const count=Number(game.media_intersection.overall_count||0);if(count>0)withCoverage++;if(Number(game.media_intersection.region_counts?.cis||0)>0)cisCoverage++;maxIntersection=Math.max(maxIntersection,count)}
