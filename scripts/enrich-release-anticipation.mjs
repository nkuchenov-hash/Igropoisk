...
let releases=read('data/releases/current.json');
const popular=read('data/popular/current.json');
const popularConfig=read('config/parsers/popular.json');
const releaseCalendarPolicy=read('config/release-calendar.json');
...
const now=Date.now();
const steamDiscovery=await enrichRawReleasesFromSteamEditorial(Array.isArray(releases?.releases)?releases.releases:[],releaseCalendarPolicy,checkedAt);
releases={...releases,releases:steamDiscovery.releases};
...
async function fetchText(url){const response=await fetch(url,{signal:AbortSignal.timeout(18000),headers:{'user-agent':'Mozilla/5.0 IgropoiskReleaseAnticipation/6.3','accept-language':'en-US,en;q=0.9'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);return response.text()}
...
releases.anticipation={measured_at:checkedAt,steam_popular_upcoming_count:steamUpcomingPositions.size,steam_popular_new_count:steamNewPositions.size,steam_editorial_discovered:steamDiscovery.discovered,popular_snapshot_generated_at:popular.generated_at||null,steam_status:steamStatus,press_status:pressStatus,press_candidates_researched:researchCandidates.length,homepage_policy:'Release notability is measured separately: broad global attention, established niche/franchise attention, and strong regional audience attention. Store rank and hosted official trailers are discovery signals, not independent coverage.'};
write('data/releases/current.json',releases);
write('data/parser-runs/release-anticipation.json',{schema_version:8,status:steamStatus==='success'&&pressStatus==='success'?'success':'partial',checked_at:checkedAt,steam_editorial_discovered:steamDiscovery.discovered,steam_popular_upcoming_count:steamUpcomingPositions.size,steam_popular_new_count:steamNewPositions.size,press_candidates_researched:researchCandidates.length,releases_enriched:(releases.releases||[]).length,popular_matches:(releases.releases||[]).filter(game=>game.anticipation?.popular_index!==null).length,homepage_eligible:eligibleGames.length,errors:[steamError,pressError].filter(Boolean)});
console.log(JSON.stringify({status:steamStatus==='success'&&pressStatus==='success'?'success':'partial',steam_editorial_discovered:steamDiscovery.discovered,steam_popular_upcoming:steamUpcomingPositions.size,steam_popular_new:steamNewPositions.size,press_candidates_researched:researchCandidates.length,releases_enriched:(releases.releases||[]).length,homepage_eligible:eligibleGames.length},null,2));
