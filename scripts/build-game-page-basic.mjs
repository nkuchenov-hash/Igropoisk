...
const blockedGalleryAsset=value=>/storepagebackground\/app\/|page_bg_generated|\/header(?:_alt)?\.(?:jpe?g|png|webp)(?:[?#]|$)/i.test(String(value||''));
const firstClean=(...values)=>values.find(value=>value&& !blockedGalleryAsset(value))||'';
...
const quality=readJSON('config/game-page-quality-v2.json',{});
...
const canonicalReleaseDate=first(...(entity.releases||[]).map(item=>item?.date?.value));
const releaseDate=first(canonicalReleaseDate,parser?.release?.canonical_date_text,parser?.release?.date_text,'Уточняется');
const year=Number(String(releaseDate).match(/(?:19|20)\d{2}/)?.[0])||null;
const modernYearFloor=Number(quality?.images?.modern_year_floor||2010);
const historical=Boolean(year&&year<modernYearFloor);
const minimumScreenshots=historical?Number(quality?.images?.screenshot?.minimum_count_historical||5):Number(quality?.images?.screenshot?.minimum_count_modern||config?.page_gate?.minimum_screenshots||6);
...
const steamLibraryHero=steamAppId?`https://cdn.cloudflare.steamstatic.com/steam/apps/${steamAppId}/library_hero.jpg`:'';
const steamLibraryCover=steamAppId?`https://cdn.cloudflare.steamstatic.com/steam/apps/${steamAppId}/library_600x900.jpg`:'';
const cover=first(firstClean(parser?.media?.cover,...media(entity,['cover','keyArt'])),steamLibraryCover);
const hero=first(firstClean(parser?.media?.hero,...media(entity,['hero','keyArt'])),steamLibraryHero,cover);
const artwork=unique([...(parser?.media?.artwork||[]),...media(entity,['keyArt','hero'])]).filter(value=>!blockedGalleryAsset(value));
...
if(!publicReady){
  ...
  console.log(JSON.stringify({slug,game_id:entity.id,mode:'structured_sources',year,sources:sources.length,screenshots:screenshots.length,minimum_screenshots:minimumScreenshots,media_profile:historical?'historical':'modern',gate_passed:true,public_ready:false,next:'review/rating quality loop'},null,2));
  process.exit(0);
}
...
console.log(JSON.stringify({slug,game_id:entity.id,mode:'structured_sources',year,sources:sources.length,screenshots:screenshots.length,minimum_screenshots:minimumScreenshots,media_profile:historical?'historical':'modern',gate_passed:true,public_ready:true},null,2));
