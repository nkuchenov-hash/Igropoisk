...
const canonicalRelease=(entity.releases||[]).find(item=>item.date?.value)?.date?.value||field(entity,'releaseDate','')||field(entity,'releaseYear','');
const releaseDate=first(canonicalRelease,parser?.release?.date_text,parser?.release?.date,'Уточняется');
