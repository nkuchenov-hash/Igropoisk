...
const sources = [
  { source: 'PlayStation Blog', publisher: 'Sony Interactive Entertainment', url: 'https://blog.playstation.com/feed/' },
  { source: 'Xbox Wire', publisher: 'Microsoft Gaming', url: 'https://news.xbox.com/en-us/feed/' },
  { source: 'Nintendo', publisher: 'Nintendo', url: 'https://www.nintendo.co.jp/news/whatsnew.xml' },
  { source: 'Steam News', publisher: 'Valve', url: 'https://store.steampowered.com/feeds/news.xml' }
];

const outputPath = path.resolve('data/publisher-news.json');
const imageDirectory = path.resolve('assets/publisher-news');
const userAgent = 'IgropoiskPublisherBot/1.0 (+https://github.com/nkuchenov-hash/Igropoisk)';
const maxItems = 80;
const maxAgeDays = 14;
...
function parseFeed(xml, source) {
  ...
  return blocks.slice(0, 30).map(block => {
    ...
    const url = absolute(strip(tag(block, ['link'])) || attr(linkTag, 'href'), source.url);
    const summary = strip(tag(block, ['description', 'summary', 'content:encoded', 'content'])).slice(0, 320);
    ...
    return { id: createHash('sha1').update(url || `${source.source}-${title}`).digest('hex').slice(0, 16), title, summary, url, publishedAt, source: source.source, publisher: source.publisher };
...
async function translate(text, target) {
  ...
  const { text: body } = await fetchText(endpoint, 15000);
...
function extractImage(html, articleUrl) {
  ...
  for (const key of ['og:image:secure_url', 'og:image', 'twitter:image', 'twitter:image:src']) {
    for (const meta of metas) {
      ...
      if (name === key) return absolute(attr(meta, 'content'), articleUrl);
    }
  }
  return '';
}
...
function extension(type, url) {
  ...
  try {
    ...
    return ['.jpg', '.jpeg', '.png', '.webp', '.avif', '.gif'].includes(ext) ? (ext === '.jpeg' ? '.jpg' : ext) : '';
  } catch { return ''; }
}
...
async function hydrate(item) {
  try {
    ...
    if (!imageUrl) throw new Error('no original image');
    const [titleRu, titleEn, summaryRu, summaryEn, downloaded] = await Promise.all([
      translate(item.title, 'ru'), translate(item.title, 'en'),
      translate(item.summary || item.title, 'ru'), translate(item.summary || item.title, 'en'),
      downloadImage(imageUrl, item.id, finalUrl)
    ]);
    if (!titleRu || !titleEn) throw new Error('translation unavailable');
    return { ...item, url: finalUrl, type: 'publisher', official: true, titleRu, titleEn, summaryRu, summaryEn, ...downloaded, homeUntil: new Date(new Date(item.publishedAt).getTime() + 36 * 3600e3).toISOString() };
  } catch (error) {
    console.error(`[publisher] ${item.url}: ${error.message}`);
    return null;
  }
}
...
for (const source of sources) {
  try { raw.push(...parseFeed((await fetchText(source.url, 15000)).text, source)); }
  catch (error) { console.error(`[publisher/feed] ${source.source}: ${error.message}`); }
}
...
await fs.writeFile(outputPath, `${JSON.stringify({ generatedAt: new Date().toISOString(), updateFrequency: 'hourly', items }, null, 2)}\n`);
console.log(`[publisher] wrote ${items.length} official items`);
