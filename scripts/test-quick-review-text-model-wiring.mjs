...
if(!startBlock.includes('continue-on-error: true')||!startBlock.includes('id: full_model_service'))fail('Optional local model service can still fail the commercial run');
...
if(!fullBlock.includes('continue-on-error: true')||!fullBlock.includes('id: full_upgrade'))fail('Optional full-review upgrade can still fail the commercial run');
const publishBlock=workflow.slice(workflow.indexOf('- name: Publish full-review checkpoint conflict-safely'));
if(!publishBlock.includes("if: steps.full_upgrade.outcome == 'success'"))fail('Failed optional full-review output can still be published');

console.log('Quick-review model wiring passed: commercial bootstrap is deterministic, live-proven and model-independent; all local qwen/full-review work is optional and cannot invalidate the commercial run.');
