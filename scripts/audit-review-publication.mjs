...
for(const game of catalog){
  ...
  const draftIdentityOk=!draft||!draftTitle||draftTitle===expected;
  ...
  const reasons=[];
  if(draft&&!draftIdentityOk)reasons.push(`draft_title_mismatch:${draft?.identity?.title||''}`);
  ...
  const row={slug,title:game.title,draft_title:draft?.identity?.title||null,page_draft:pageDraft||null,article_files:articleFiles,article_game_slug:articleGameSlug||null,review_feed: Boolean(reviewFeed),review_game_slug:reviewGameSlug||null,dynamic_runtime:dynamicRuntime,runtime_article_url:runtimeArticleUrl||null,linked_from_game_page:linked,publication_verified:userPublished,reasons};
  rows.push(row);
  for(const reason of reasons)countReason(reason);
  if(reasons.length||articleFiles&&!userPublished)blocking.push(row);
}

const report={schema_version:2,checked_at:new Date().toISOString(),catalog_games:rows.length,article_file_pairs:rows.filter(r=>r.article_files).length,verified_published_reviews:rows.filter(r=>r.publication_verified).length,blocking:blocking.length,reason_counts:reasonCounts,rows};
