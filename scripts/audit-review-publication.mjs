...
for(const game of catalog){
  ...
  if(articlePublishedFlag&&!linked)reasons.push('article_not_exposed_by_game_page');
  if(intentionallyWithheld&&linked)reasons.push('withheld_article_exposed_by_game_page');
  const userPublished=articlePublishedFlag&&articleSlugOk&&draftIdentityOk&&articlePage&&linked;
...
const report={schema_version:5,checked_at:new Date().toISOString(),catalog_games:rows.length,article_json_records:rows.filter(r=>r.article_json).length,verified_published_reviews:rows.filter(r=>r.publication_verified).length,withheld_reviews:rows.filter(r=>r.withheld_clean).length,blocking:blocking.length,reason_counts:reasonCounts,rows};
...
console.log(JSON.stringify({catalog_games:report.catalog_games,verified_published_reviews:report.verified_published_reviews,withheld_reviews:report.withheld_reviews,blocking:report.blocking,reason_counts:reasonCounts,examples:blocking.slice(0,15).map(r=>({slug:r.slug,reasons:r.reasons}))},null,2));
