...
const prepareScript=firstExisting([
  'scripts/build-review-from-request.mjs',
  'scripts/generate-review-article.mjs',
  'scripts/synthesize-review.mjs',
  'scripts/build-review-draft.mjs'
]);
...
for(const game of queue){
  ...
  const request={slug,title:game.title,year:game.year,requested_at:new Date().toISOString(),scope:'full_review_article',source:'catalog-visible',rebuild:true,requirements:{design_system_only:true,hero_art_only:true,minimum_unique_screenshots_per_section:3,no_repeated_scenes:true}};
  writeJson(`data/review-build-requests/${slug}.json`,request);
  const startedAt=new Date().toISOString();
  try{
    const hasDraft=exists(`data/drafts/${slug}.json`);
    const hasArticle=exists(`data/articles/${slug}.json`)||exists(`data/article-drafts/${slug}.json`);
    if((!hasDraft||!hasArticle)&&!prepareScript)throw new Error('No article preparation script exists for a game without draft/article data');
    if(prepareScript)run(prepareScript,[slug]);
  ...
  writeJson('data/parser-runs/review-batch-latest.json',{schema_version:1,updated_at:new Date().toISOString(),total:queue.length,completed:results.filter(item=>item.status==='success').length,failed:results.filter(item=>item.status==='failed').length,results});
}
