...
if(existing?.publication_status==='published'&&existing?.generation?.grounding_audit?.passed===true&&existing?.generation?.editorial_quality?.passed===true&&Number(existing.score)===score&&existingWords>=220){
  ...
  console.log(JSON.stringify({slug,status:'already_commercial',words:existingWords,model:existing.generation?.model||null},null,2));process.exit(render.status??1);
}
fs.rmSync(articlePath,{force:true});
const synthesis=spawnSync('node',['scripts/build-review-bootstrap-commercial-local.mjs',slug],{cwd:root,encoding:'utf8',stdio:'inherit',env:process.env,maxBuffer:32*1024*1024});
...
if(words<220)throw new Error(`${slug}: direct commercial quick review below 220 words (${words})`);
...
const audited=read(relative,{});
if(audited?.publication_status!=='published'||audited?.generation?.grounding_audit?.passed!==true||audited?.generation?.editorial_quality?.passed!==true)throw new Error(`${slug}: factual/language audit returned without a publishable commercial review`);
const render=spawnSync('node',['scripts/render-review-bootstrap.mjs',slug],{cwd:root,encoding:'utf8',stdio:'inherit',env:process.env,maxBuffer:32*1024*1024});
