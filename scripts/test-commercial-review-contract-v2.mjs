...
for(const marker of ['meta_parts','MAX_LEAD_PARTS=3','LEAD_PART_TIMEOUT_MS=90000','LEAD_PART_RETRY_TIMEOUT_MS=60000','LEAD_SENTENCE_PACK_TIMEOUT_MS=75000','LEAD_SENTENCE_PACK_RETRY_TIMEOUT_MS=60000','LEAD_GENERATION_NUM_CTX=4096','LEAD_PART_NUM_PREDICT=420','LEAD_SENTENCE_PACK_NUM_PREDICT=360','DEK_TIMEOUT_MS=60000','META_QUALITY_TIMEOUT_MS=90000','SECTION_REUSE_AUDIT_TIMEOUT_MS=90000','META_AUDIT_NUM_CTX=4096','META_AUDIT_NUM_PREDICT=320','SECTION_REUSE_AUDIT_NUM_CTX=4096','SECTION_REUSE_AUDIT_NUM_PREDICT=320','MAX_META_QUALITY_PASSES=2','compactLeadSource','compactLeadEvidence','compactAuditSource','compactAuditEvidence','leadIdentity','generateLeadPart','generateLeadSentencePack','recoverLeadPart','sentencePackSchema','minItems:3','maxItems:4','leadPartGate','dedupeLeadParts','nearDuplicateText','paragraphDuplicateErrors','auditMeta','auditPersistedSection','rejectPersistedSection','quality_reuse_audit','section_quality_rejections','persistent-multi-paragraph-meta-v7-compact-audits-quality-gated','structured_sentence_pack_recovery:true','compact_cpu_lead_recovery:true','compact_cpu_quality_audits:true'])assert.ok(metaPreflight.includes(marker),`persisted editorial quality/recovery safeguard missing: ${marker}`);
...
assert.ok(metaPreflight.includes('COMPACT EVIDENCE FROM ALL ${compactAuditEvidence.length} ACCEPTED PROFESSIONAL SOURCES'),'meta audit must preserve corpus-wide grounding while using compact evidence');
assert.ok(metaPreflight.includes('numCtx:META_AUDIT_NUM_CTX'),'meta quality audit must use the compact CPU context budget');
assert.ok(metaPreflight.includes('numPredict:META_AUDIT_NUM_PREDICT'),'meta quality audit must use the compact CPU output budget');
assert.ok(metaPreflight.includes('numCtx:SECTION_REUSE_AUDIT_NUM_CTX'),'persisted-section quality audit must use the compact CPU context budget');
assert.ok(metaPreflight.includes('numPredict:SECTION_REUSE_AUDIT_NUM_PREDICT'),'persisted-section quality audit must use the compact CPU output budget');
assert.ok(metaPreflight.includes("if(deterministic.length)return rejectPersistedSection"),'obviously broken persisted sections must be rejected before spending a model audit');
...
assert.ok(!metaPreflight.includes('numCtx:8192,numPredict:650'),'heavy meta quality audit request was reintroduced');
assert.ok(!metaPreflight.includes('numCtx:8192,numPredict:700'),'heavy persisted-section audit request was reintroduced');
...
for(const marker of ["'qwen3:4b'",'repeatPenalty=1.18','repeatLastN=1024','strengthenJsonSchema','isCompactBooleanAuditSchema','boundedJsonPredictBudget','return Math.min(requested, 256)'])assert.ok(local.includes(marker),`local editorial transport contract missing: ${marker}`);
...
console.log('Commercial review contract v2 static boundary passed, including provider-independent fallback, bounded research, compact CPU structured lead recovery, compact corpus-wide meta and persisted-section quality audits, deterministic rejection of obviously broken persisted sections, compact CPU section continuation, local-first persistent synthesis, non-destructive commercial media bootstrap, non-empty structured fields and per-request concurrency.');
