...
assert.ok(postRunner.includes("else run(`media:${slug}`,'scripts/enrich-game-media-from-sources.mjs',[slug])"),'bootstrap must still enrich games that have not yet passed the commercial media gate');
...
console.log('Commercial review contract v2 static boundary passed, including provider-independent fallback, bounded research, compact split meta preflight, local-first persistent sectioned synthesis, non-destructive commercial media bootstrap, non-empty structured fields and per-request concurrency.');
