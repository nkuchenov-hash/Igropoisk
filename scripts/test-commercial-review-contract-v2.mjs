...
for(const marker of ['meta_parts','MAX_LEAD_PARTS=3','LEAD_PART_TIMEOUT_MS=150000','LEAD_PART_RETRY_TIMEOUT_MS=90000','DEK_TIMEOUT_MS=60000','generateLeadPart','partErrors','joinedLead','slice(partIndex*2,partIndex*2+2)','numCtx:6144','numPredict:520','leadMinWords','nearDuplicate','duplicateLeadPair','partErrors(part,currentLead)','persistent-multi-paragraph-meta-v4-deduped','near_duplicate_lead:false'])assert.ok(metaPreflight.includes(marker),`persistent deduplicated multi-paragraph meta safeguard missing: ${marker}`);
...
assert.ok(metaPreflight.includes('retained.some(existing=>nearDuplicate(part,existing))'),'persisted near-duplicate lead parts must be discarded before continuation');
...
console.log('Commercial review contract v2 static boundary passed, including provider-independent fallback, bounded research, persistent deduplicated multi-paragraph meta, bounded section continuation, local-first persistent sectioned synthesis, non-destructive commercial media bootstrap, non-empty structured fields and per-request concurrency.');
