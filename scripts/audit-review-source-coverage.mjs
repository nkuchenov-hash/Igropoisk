...
const rows=editorialSources(registry,{historical:true}).map(source=>{
 const applicable=!source.historical_only||historical,item=bySource.get(source.id),check=checks.get(source.id),score=Number(item?.score),scale=Number(item?.scale),hasExplicitScore=Number.isFinite(score)&&Number.isFinite(scale)&&scale>0,canonicalVersionEligible=item?.canonical_score_eligible!==false,scoreEligible=canonicalVersionEligible&&item?.score_eligible!==false&&(hasExplicitScore||Boolean(item?.grade));
 return {source_id:source.id,publication:source.name,language:source.language,regions:source.regions,applicable,corpus_status:!applicable?'not_applicable':item?'found':check?.status||'unchecked',review_url:item?.resolved_url||item?.url||'',source_kind:item?.source_kind||'',canonical_score_eligible:canonicalVersionEligible,version_reason:item?.version_validation?.reason||'',score:hasExplicitScore?score:null,scale:hasExplicitScore?scale:null,score_10:hasExplicitScore?Number((score/scale*10).toFixed(4)):null,score_eligible:scoreEligible,grade:item?.grade||'',validation_status:item?.validation?.status||'',check_notes:!applicable?'historical-only source outside game era':check?.notes||''};
});
