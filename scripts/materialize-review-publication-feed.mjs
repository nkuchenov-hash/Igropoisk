...
const catalog=read('data/catalog-visible.json',[]),out={schema_version:6,checked_at:new Date().toISOString(),updated:[],cleared:[],valid:[],needs_revision:[]};
function reject(slug,feed,reason){
  ...
  out.needs_revision.push({slug,reason,bootstrap_publication_forbidden:true});
}
for(const game of catalog){
  const slug=String(game.slug||'');if(!slug)continue;
  const relative=`data/reviews/${slug}.json`,feed=read(relative,{schema_version:6,game_slug:slug,reviews:[]}),article=read(`data/articles/${slug}.json`),commercial=read(`data/parser-runs/review-commercial-v2-${slug}.json`);let reason='';
  ...
  else if(commercial?.passed!==true)reason='commercial_review_v2_not_green';
  ...
  const same=feed.igropoisk_article&&feed.igropoisk_article.url===desired.url&&Number(feed.igropoisk_article.score)===score&&feed.igropoisk_article.score_source===desired.score_source&&feed.igropoisk_article.review_stage==='full'&&feed.igropoisk_article.commercial_review_status==='green';
...
console.log(JSON.stringify({dry_run:dry,updated:out.updated.length,cleared:out.cleared.length,valid:out.valid.length,needs_revision:out.needs_revision.length,bootstrap_publication:false,top_250_count:top250?.count??null},null,2));
