...
for(const marker of ['PUBLISH_MERGE_ATTEMPTS','POST_CREATE_PUBLISH_MERGE_ATTEMPTS||12','PUBLISH_SETTLE_MS','POST_CREATE_PUBLISH_SETTLE_MS||2500','mergeBackoff','publishAttempt<=PUBLISH_MERGE_ATTEMPTS','await sleep(PUBLISH_SETTLE_MS)','rebuilding on newest staging after backoff','publish_attempt_limit:PUBLISH_MERGE_ATTEMPTS'])if(!overlay.includes(marker))fail(`Checkpoint publisher lacks active-staging churn tolerance: ${marker}`);
if(overlay.includes('publishAttempt<=5'))fail('Checkpoint publisher reverted to the too-short five-attempt merge window');
for(const marker of ['repository_dispatch:','types: [production-pages]','github.event.client_payload.production_sha || github.sha'])if(!pages.includes(marker))fail(`Pages exact-SHA dispatch contract missing: ${marker}`);if(overlay.includes("gh',['workflow','run'"))fail('Post-create production must not depend on recursively triggered workflow events');console.log('Direct fail-closed exact-SHA post-create production publisher contract passed with bounded active-staging churn tolerance.');
