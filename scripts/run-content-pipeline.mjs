...
for(const task of plan.pages||[]){
  ...
  if(!parsed)continue;
  run(`page:${task.slug}`,'node',['scripts/build-game-page.mjs',task.slug],{OPENAI_MODEL:process.env.OPENAI_MODEL||'gpt-5',OPENAI_RESEARCH_MODEL:process.env.OPENAI_RESEARCH_MODEL||process.env.OPENAI_MODEL||'gpt-5'});
}
...
writeJSON('data/content-pipeline/execution-log.json',{schema_version:1,started_at:startedAt,finished_at:finishedAt,summary,results});
