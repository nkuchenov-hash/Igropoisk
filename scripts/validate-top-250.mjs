...
for (let i = 0; i < ranking.length; i += 1) {
  ...
  if (fs.existsSync(reviewData) && fs.existsSync(articleData)) {
    ...
    if (!Number.isFinite(canonicalScore) || canonicalScore !== score || Number(article.score) !== score) errors.push(`Top-250 score is not identical to canonical review score for ${item.slug}`);
  }
}
