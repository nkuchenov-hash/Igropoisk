...
const references = collectNewsGamePageReferences({ items: [
  { publishedAt: '2026-08-10T10:00:00Z', games: [{ gameId: 'game_alpha', slug: 'alpha' }] },
...
assert.equal(references.get('game_alpha').mentions, 2);
...
assert.equal(references.get('game_gamma').mentions, 1);
...
assert.equal(runnable.filter(item => item.news_reference).length, 2, 'All news-triggered page tasks must bypass the normal page-per-run cap.');
