...
async function inspect(item){
  ...
  try{
    ...
    if(declared&&declared>maxBytes)return {...item,file_probe_passed:false,reject_reason:'file_too_large',bytes:declared,mime:type};
...
function add(item,kind='screenshot'){
  ...
  raw.push({id:`shot-${raw.length+1}`,url:normalized.url,caption:normalized.caption||'Скриншот игры',source_url:sourceUrl,source_domain:host(sourceUrl)||host(normalized.url),source_name:normalized.source_name||host(sourceUrl)||host(normalized.url),visual_tags:normalized.visual_tags||[]});
}
