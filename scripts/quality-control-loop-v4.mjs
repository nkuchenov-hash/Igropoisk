...
function tryOpenAiThenLocal(label,openAiScript,localScript){let result=null;if(process.env.OPENAI_API_KEY&&fs.existsSync(path.join(root,openAiScript)))result=run(`${label}-openai`,openAiScript,[slug]);if((!result||result.status!==0)&&fs.existsSync(path.join(root,localScript)))result=run(`${label}-local`,localScript,[slug]);return result}
...
for(let attempt=1;attempt<=attempts;attempt++){
  ...
  if(ok){const media=tryOpenAiThenLocal(`media-enrichment-${attempt}`,'scripts/enrich-review-media.mjs','scripts/enrich-review-media-local.mjs');if(!media||media.status!==0){ok=false;comments=[(media?.stderr||media?.stdout||'No available visual audit engine').slice(-5000)]}}
  if(ok){const language=tryOpenAiThenLocal(`language-audit-${attempt}`,'scripts/audit-review-language.mjs','scripts/audit-review-language-local.mjs');if(!language||language.status!==0){ok=false;comments=[(language?.stderr||language?.stdout||'Editorial language audit failed').slice(-5000)]}}
