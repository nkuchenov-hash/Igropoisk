...
const trigger=process.env.GITHUB_SHA||'';
const runId=process.env.GITHUB_RUN_ID||String(Date.now());
const runAttempt=process.env.GITHUB_RUN_ATTEMPT||'1';
const branch=`automation/post-create-production-${runId}-${runAttempt}`;
const allowed=[
  /^data\/drafts\/[^/]+\.json$/,
...
async function createPr(body){let last='';for(let attempt=1;attempt<=5;attempt++){const result=cmd('gh',['pr','create','--base','main','--head',branch,'--title',`Publish post-create game enrichment ${runId}.${runAttempt}`,'--body',body]);if(result.status===0)return String(result.stdout||'').trim();last=String(result.stderr||result.stdout||'').trim();if(!transient(last)||attempt===5)break;await sleep(attempt*2500)}throw new Error(`Unable to create production PR: ${last}`)}

if(!trigger)throw new Error('GITHUB_SHA is required');
...
git(['commit','-m',`Publish post-create game enrichment from ${trigger.slice(0,12)}`]);
...
const payload=JSON.stringify({event_type:'production-pages',client_payload:{target_branch:'main',mode:'production',sha:mainSha,source:'post-create-production',run_id:runId,run_attempt:runAttempt,trigger_sha:trigger}});
...
console.log(JSON.stringify({status:'published',trigger,subject,production_pr:pr,main_sha:mainSha,files,rejected,pages_dispatch:'production-pages'},null,2));
